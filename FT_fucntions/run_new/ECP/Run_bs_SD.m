...
cd(bsdir)
savetag2 = [tag,'_',run,'_',subj,'_', stag,'_IC_data.mat'];
if exist(fullfile(bsdir,savetag2), 'file') ~= 2
    
    %%
    helptag = 'SM'; bsdatadir = fullfile(bsdatadir_help, 'data');
    d = rdir(fullfile(bsdatadir,subj,['/*',helptag,'*'],'/channel_vectorview306*.mat'));
    if isempty(d)
        d = rdir(fullfile(bsdatadir_help,subj,['/*',helptag,'*'],'/channel_vectorview306*.mat'));
...
%             sel = length(nn);
        ...
        %%
        savetag1 = [tag,'_',run,'_',subj,'_',stag,'_BS_channels'];
        savetag2 = [tag,'_',run,'_',subj,'_', stag,'_IC_data'];
