function [net_sel_mutiple_label, LI_sub] = do_group_LI_net_baseline_parcel(cfg_main)
    ...
    for j=1:length(net_sel_mutiple_label)
        ...
        for i=1:length(sFiles_in{1}.sFiles_in)
            ...
            if contains(method, 'Magnitude')
                [LI, ~, pow] = do_lat_analysis_asymetric_magnitude2_parcel(cfg);
                pow_sub(j,i,:) = pow;
            elseif contains(method, 'Counting')
                [LI, ~] = do_lat_analysis_DICS_baseline_Counting_parcel(cfg);
            elseif contains (method, 'Bootstrapping')
                ...
                [LI, ~] = do_LI_bootstrap_contrast_parcel(cfg);
            end
            %             LI_count = LI;
