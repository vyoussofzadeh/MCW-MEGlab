function out = do_conn_semantic(cfg_main, cln_data)
...
% % figure, plot((net.(netpar)(seed_idx,:))),
% set(gca,'Xtick', f_idx,'XtickLabel',f_range(f_idx));
% set(gca,'Ytick', 1:size(net.(netpar),1),'YtickLabel',1:size(net.(netpar),1));
% set(gca,'FontSize',9,'XTickLabelRotation',90);
...
aedge1(isnan(aedge1))=0; tedge = (aedge1.* double(aedge1 > 0.1.*max(aedge1(:))));
...
% for j = 1:length(roiidx)
%     for i = 1:l
%         idx = atlas.tissue == roiidx(j);
