function sFiles = bs_lcmv(cfg, sFiles)
% Script generated by Brainstorm (08-Mar-2022)

% % Process: Compute covariance (noise or data)
% sFiles = bst_process('CallProcess', 'process_noisecov', sFiles, [], ...
%     'baseline',       [-0.3, -0.0005], ...
%     'datatimewindow', [-2.831068713e-15, 2], ...
%     'sensortypes',    'MEG', ...
%     'target',         2, ...  % Data covariance      (covariance over data time window)
%     'dcoffset',       1, ...  % Block by block, to avoid effects of slow shifts in data
%     'identity',       0, ...
%     'copycond',       0, ...
%     'copysubj',       0, ...
%     'copymatch',      0, ...
%     'replacefile',    1);  % Replace
% 
% % Process: Compute covariance (noise or data)
% sFiles = bst_process('CallProcess', 'process_noisecov', sFiles, [], ...
%     'baseline',       [-0.3, -0.0005], ...
%     'datatimewindow', [-2.831068713e-15, 2], ...
%     'sensortypes',    'MEG', ...
%     'target',         1, ...  % Noise covariance     (covariance over baseline time window)
%     'dcoffset',       1, ...  % Block by block, to avoid effects of slow shifts in data
%     'identity',       0, ...
%     'copycond',       0, ...
%     'copysubj',       0, ...
%     'copymatch',      0, ...
%     'replacefile',    1);  % Replace

% Process: Compute covariance (noise or data)
sFiles = bst_process('CallProcess', 'process_noisecov', sFiles, [], ...
    'baseline',       [0, 1], ...
    'datatimewindow', [0, 1], ...
    'sensortypes',    'MEG', ...
    'target',         1, ...  % Noise covariance     (covariance over baseline time window)
    'dcoffset',       1, ...  % Block by block, to avoid effects of slow shifts in data
    'identity',       0, ...
    'copycond',       0, ...
    'copysubj',       0, ...
    'copymatch',      0, ...
    'replacefile',    1);  % Replace

% Process: Compute covariance (noise or data)
sFiles = bst_process('CallProcess', 'process_noisecov', sFiles, [], ...
    'baseline',       [0, 1], ...
    'datatimewindow', [0, 1], ...
    'sensortypes',    'MEG', ...
    'target',         2, ...  % Noise covariance     (covariance over baseline time window)
    'dcoffset',       1, ...  % Block by block, to avoid effects of slow shifts in data
    'identity',       0, ...
    'copycond',       0, ...
    'copysubj',       0, ...
    'copymatch',      0, ...
    'replacefile',    1);  % Replace

% Process: Average: Everything
sFiles_avg = bst_process('CallProcess', 'process_average', sFiles, [], ...
    'avgtype',         1, ...  % Everything
    'avg_func',        1, ...  % Arithmetic average:  mean(x)
    'weighted',        0, ...
    'scalenormalized', 0);

sFiles_avg_toffset = bst_process('CallProcess', 'process_timeoffset', sFiles_avg, [], ...
    'info',      [], ...
    'offset',    0.016, ...
    'overwrite', 0);


% Process: Compute sources [2018]
sFiles = bst_process('CallProcess', 'process_inverse_2018', sFiles_avg_toffset, [], ...
    'output',  1, ...  % Kernel only: shared
    'inverse', struct(...
    'Comment',        cfg.comment, ...
    'InverseMethod',  'lcmv', ...
    'InverseMeasure', 'nai', ...
    'SourceOrient',   {{'fixed'}}, ...
    'Loose',          0.2, ...
    'UseDepth',       1, ...
    'WeightExp',      0.5, ...
    'WeightLimit',    10, ...
    'NoiseMethod',    'median', ...
    'NoiseReg',       0.1, ...
    'SnrMethod',      'rms', ...
    'SnrRms',         1e-06, ...
    'SnrFixed',       3, ...
    'ComputeKernel',  1, ...
    'DataTypes',      {{'MEG GRAD', 'MEG MAG'}}));

% Process: Z-score transformation: [-300ms,-1ms]
% sFiles = bst_process('CallProcess', 'process_baseline_norm', sFiles, [], ...
%     'baseline',   [-0.3, -0.0005], ...
%     'source_abs', 0, ...
%     'method',     'zscore', ...  % Z-score transformation:    x_std = (x - &mu;) / &sigma;
%     'overwrite',  1);

end
