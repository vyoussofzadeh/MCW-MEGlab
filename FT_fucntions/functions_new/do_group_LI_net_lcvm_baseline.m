function [net_sel_mutiple_label, LI_sub] = do_group_LI_net_lcvm_baseline(cfg_main)
...
if exist(savefilename,'file') == 2
    ...
    for j=1:length(net_sel_mutiple_label)
        ...
        for i=1:length(sFiles_in)
            ...
            switch method
                case 'LCMV_contrast_Magnitude'
                    [LI, ~, pow] = do_lat_analysis_asymetric_magnitude(cfg);
                    pow_sub(j,i,:) = pow;
                case 'LCMV_basline_Counting'
                    [LI, ~] = do_lat_analysis_baseline_Counting(cfg);
                case 'LCMV_contrast_Counting'
                    [LI, ~] = do_lat_analysis_DICS_contrast_Counting(cfg);
                case 'LCMV_bootstrapping'
                        cfg.divs = 10;
