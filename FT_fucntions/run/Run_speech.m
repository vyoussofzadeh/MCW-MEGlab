...
%% Convert speech to audio, failed...
% sound(tmp);
% y = upsample(speech_data.trial{1,i},44e3/1000);
% y = y./max(y);
% audiowrite('handel.wav',y,44e3)
% Fs = 1e3;
% soundsc(speech_data.trial{1,i}, 2*Fs, 16);
% 
% y1 = speech_data.trial{1,i};
% % y2 = y1/max(y1);
% y3 = 1e5*y1;
% figure,plot(y3)
% 
% y = int16(y3);
% y1 = upsample(y,44e3/1000);
% audiowrite('test.wav', y1, 44e3);
% player = audioplayer(y1, 44e3);

% audiowrite('test_low.wav', y, Fs);
%%
