...
datacheck = fullfile(bsdir,'data',D.sub{df},[D.sub{df},'_',D.session{df},'_', stag, '*'],'*.mat');
d = rdir(datacheck);

savetag2 = [D.sub{df},'_',D.session{df},'_', stag,'_IC_data.mat'];
% if exist(fullfile(bsdir,savetag2), 'file') ~= 2
subj = ['Subject', D.sub{df}];
tag = stag;
...
if isempty(d)
    ...
    if ~isempty(d)
        ...
        %%
        bssavedir = fullfile(bsdir,'ft_export');
