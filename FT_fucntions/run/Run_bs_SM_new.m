...
cd(bsdir)
savetag2 = [tag.task,'_',subj,'_', run, '_', tag.dcon, '_IC_data.mat'];
if exist(fullfile(bsdir,savetag2), 'file') ~= 2
    
    %%
    bsdatadir = fullfile(bsdatadir_help, 'data');
    d = rdir(fullfile(bsdatadir,subj,['/*',tag.task,'*'],'/channel_vectorview306*.mat'));
    if isempty(d)
        d = rdir(fullfile(bsdatadir_help,subj,['/*',tag.task,'*'],'/channel_vectorview306*.mat'));
    ...
    if ~isempty(d)
        ...
        if length(nn)>1
            ...
            for dpick=1:length(nn)
                ...
                if ~isempty(Index), sel = dpick; break,
                else
                    sel = 1;
                end
            end
        ...
        %%
        savetag1 = [tag.task,'_',subj,'_',run, '_', tag.dcon, '_BS_channels'];
        savetag2 = [tag.task,'_',subj,'_', run, '_', tag.dcon, '_IC_data'];
        
%         savetag1 = [tag.task,'_',subj,'_',run, '_', tag.dcon, '_BS_channels1'];
%         savetag2 = [tag.task,'_',subj,'_', run, '_', tag.dcon, '_IC_data1'];
        ...
        %%
        savetag3 = [tag.task,'_',subj,'_', run, '_', tag.dcon, '_ltrl'];
        ltrl = [length(ep_data_str_math.pst.trial),length(ep_data_str_math.bsl.trial)];
        save(fullfile(bsdir,savetag3),'ltrl');
