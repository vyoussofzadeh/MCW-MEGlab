...
[f_data.pst,~,~,tapsmofrq] = vy_fft(cfg, ep_data); f_data.pst.elec = cfg_main.sens;


clear('input')
f_bsl = input('Baseline freq? '); 

cfg = [];
cfg.savefile = [];
cfg.saveflag = 2;
cfg.foilim = [f_bsl f_bsl];
cfg.plotflag  = 2;
cfg.taper    = 'dpss'; cfg.tapsmofrq  = tapsmofrq;
[f_data.bsl,~,~,tapsmofrq] = vy_fft(cfg, ep_data); f_data.bsl.elec = cfg_main.sens;
