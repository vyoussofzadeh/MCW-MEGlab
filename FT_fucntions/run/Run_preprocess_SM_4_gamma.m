...
if exist(fullfile(outd.sub,['ic_',subj,'.mat']), 'file') == 2
    ...
    if flag.preprocessing.filtering == 1
        ...
        if exist(savepath, 'file') == 2
            ...
            cfg.trialdef.cutmode = 1;   % 1: TEV, 2: TRESP, 3: BSENT, 4: BUN, see, trialfun_StoryM_BaseExtractAll_neuromag    
            cfg = ft_definetrial(cfg);
            
            %%
            idx_str = find(cfg.event(:,21)==10); idx_math = find(cfg.event(:,21)==20);
            cfg1  = cfg;
            cfg1.event = []; cfg1.event = cfg.event([idx_str;idx_math],:);
            cfg1.trl = []; cfg1.trl = cfg.trl([idx_str;idx_math],:);
            cfg = cfg1;
