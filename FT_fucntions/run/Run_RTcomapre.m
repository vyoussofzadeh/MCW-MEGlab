...
%% RT
[C1,ia1,ib1] = intersect(DFN4.RT_all.Var1,C, 'stable');

RT_DFN = DFN4.RT(ia1,1);

[C1,ia1,ib1] = intersect(PN4.RT_all.Var1,C, 'stable');
RT_PN = PN4.RT(ia1,1);

figure,
b = bar([RT_DFN(:,1) RT_PN(:,1)]);
...
idx_dfn_good = find(RT_DFN>0.7);
idx_pn_good = find(RT_PN>0.4);

RT_DFN1 = RT_DFN(idx_dfn_good);
RT_PN1 = RT_DFN(idx_pn_good);

disp([mean(RT_DFN1),std(RT_DFN1)])
disp([mean(RT_PN1),std(RT_PN1)])
