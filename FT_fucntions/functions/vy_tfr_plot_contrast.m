function [time_of_interest,freq_of_interest] = vy_tfr_plot_contrast(cfg_main, tfr1, tfr2)

%%
% First compute the average over trials:
cfg = [];
freq_avg1 = ft_freqdescriptives(cfg, tfr1);
freq_avg2 = ft_freqdescriptives(cfg, tfr2);

% freq_avg = freq_avg1;
% 
% freq_avg1.powspctrm(isnan(freq_avg1.powspctrm))=0;
% freq_avg2.powspctrm(isnan(freq_avg2.powspctrm))=0;
% freq_avg.powspctrm = (freq_avg1.powspctrm  - freq_avg2.powspctrm);
...
freq_avg_bsl1 = ft_freqbaseline(cfg, freq_avg1);
freq_avg_bsl2 = ft_freqbaseline(cfg, freq_avg2);


meanpow1 = squeeze(mean(freq_avg_bsl1.powspctrm, 1));
meanpow2 = squeeze(mean(freq_avg_bsl2.powspctrm, 1));
...
tim_interp = linspace(-0.5, 2.5, 512);
...
meanpow1(isnan(meanpow1))=0;
meanpow2(isnan(meanpow2))=0;
meanpow = (meanpow1  - meanpow2);
