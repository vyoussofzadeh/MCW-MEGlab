...
if exist(savefile, 'file') == 2
    ...
    for i=1:length(files_sel)
        ...
        pow(i,:) = nanmean(s_all,1);
        disp(datafile)
        disp(['subj:',Subj])
        disp(['run:',Run])
        disp([num2str(i),'/', num2str(length(files_sel))])
        disp('============');
        names{i} = [num2str(i),'_',Subj,'_',Run];
        Sub_all{i} = Subj;
    end
...
% %     {'EC1073'}
% 
% subj_pow_fixed = subj_pow;
% subj_pow_fixed.sub(idbx) = [];
% subj_pow_fixed.pow(idbx,:) = [];
...
Run_plot_KB_subject
Run_plot_KB_subject_stats
Run_parcellation_KB_subject

%%
Run_plot_KB_subject_run
Run_plot_KB_subject_run_stats
