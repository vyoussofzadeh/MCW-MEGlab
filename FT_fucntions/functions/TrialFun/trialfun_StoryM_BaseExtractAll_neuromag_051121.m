function    [trl,trialinfo, trlInfoColDescr, trialSummary,scanStartSamp,scanEndSamp,warninfo]  = trialfun_StoryM_BaseExtractAll_neuromag_051121( cfg )
...
% detResp=ft_read_data(datafile,'chanindx',[320],'header',hdr,'eventformat','Neuromag','dataformat','neuromag_fif');
% 
...
[~,j1]=find(ismember(detTrig(2:end),[trgST_Cue trgMA_Cue])&(difTrig>0));
...
for iBlock=1:Nblocks
    ...
    tmpIndxHeadOn=indxHeadOn((indxHeadOn>=trlBlockInfo(iBlock,2))&(indxHeadOn<trlBlockInfo(iBlock,3)));
    ...
    for iUn=1:tmpNun
        ...
        if trlUnitInfo(countUn,2)==1 % Story
            trlUnitInfo(countUn,7)=0.5*(detTrig(indxTrigUp(find((indxTrigUp>tmpIndxHeadOn(iUn)),1,'first')+1)) - trg_headblock); % added by VYZ
            ...
            %- correction by vy.
            tmp = indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==trgMA_N_base),1,'first'));
            if ~isempty(tmp)
                trlUnitInfo(countUn,9)=tmp;
            elseif ~isempty (indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==74),1,'first')))
                trlUnitInfo(countUn,9) = indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==74),1,'first'));
            elseif ~isempty (indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==72),1,'first')))
                trlUnitInfo(countUn,9) = indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==72),1,'first'));                
            end
            %-
            ...
            elseif detTrig(trlUnitInfo(countUn,9))==66 % added by vy.
                tmpCorOpt=2;
                tmpOptBase=trgMA_O_Cor2;
                tmp_O_wordon=trgMA_O_Cor2wordon;
            else
            ...
            test_11=indxTrigUp(find((indxTrigUp>tmpIndxHeadOn(iUn))&(detTrig(indxTrigUp)==76),1,'first'));
            ...
            tmp_O_WordIndx=indxTrigUp((indxTrigUp>=trlUnitInfo(countUn,11))&(indxTrigUp<trlUnitInfo(countUn,12)));
            
            %- correction by VY
            if trlUnitInfo(countUn,9) > trlUnitInfo(countUn,11)
                trlUnitInfo(countUn,9) = indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown)==72),1,'first'));
            ...
            %- correction by VY
            if isempty(tmp_O_WordIndx)
                trlUnitInfo(countUn,11)=indxTrigUp(find((indxTrigUp>tmpIndxHeadOn(iUn))&(detTrig(indxTrigUp)==76),1,'first'));
                tmp=indxTrigDown(find((indxTrigDown>trlUnitInfo(countUn,8))&(detTrig(indxTrigDown-1)==88)&indxTrigDown<test_11,end,'first'));
                trlUnitInfo(countUn,9) = tmp(end);
                tmp_O_WordIndx=indxTrigUp((indxTrigUp>=trlUnitInfo(countUn,11))&(indxTrigUp<trlUnitInfo(countUn,12)));
                
                %---
                tmp2 = tmp(end);
%                 figure,plot(diff(detTrig(tmp(end-1):tmp(end))));
                l = 1:length(detTrig);
                idx = find(diff(detTrig(tmp(end-1):tmp(end)))>0);
                l2 = l(tmp(end-1):tmp(end));
%                 figure,plot(l(l2(idx)): tmp2, detTrig(l2(idx):tmp2));
%                 hold on
%                 plot([l(l2(idx)), tmp2],[detTrig(l2(idx)) detTrig(tmp2)],'r')
%                 linspace(detTrig(l2(idx)),detTrig(a),a-l(l2(idx))+1)
                detTrig(l2(idx):tmp2) = linspace(detTrig(l2(idx)),detTrig(tmp2),tmp2-l(l2(idx))+1);
                %---
            
                tmp3 = trlUnitInfo(countUn,12);
                tmp4 = tmp3-3:tmp3+3;
                [~,a] = max(detTrig(tmp4));
                tmp5 = tmp4(a);
                tmp6 = linspace(detTrig(tmp5-1),detTrig(tmp5+1),3);
                detTrig(tmp5-1:tmp5+1)= tmp6;
                trlUnitInfo(countUn,12)= tmp3+1;
        ...
        plot([trlUnitInfo(countUn,8) trlUnitInfo(countUn,8)],[y(1) y(2)])
        plot([trlUnitInfo(countUn,9) trlUnitInfo(countUn,9)],[y(1) y(2)])
        plot([trlUnitInfo(countUn,11) trlUnitInfo(countUn,11)],[y(1) y(2)])
        plot([trlUnitInfo(countUn,12) trlUnitInfo(countUn,12)],[y(1) y(2)])
        countUn=countUn+1;
    end
end
%=====================================================
...
%     end
%     for iSub=1:length(tmpIndxSubs)
%         trlSubUnitInfo(countsubs,1:20)=trlUnitInfo(iUn,1:20);
%         trlSubUnitInfo(countsubs,21)=iSub;
%         trlSubUnitInfo(countsubs,22)=indxTrigUp(tmpIndxSubs(iSub));
%         trlSubUnitInfo(countsubs,23)=1;
%         if trlUnitInfo(iUn,2)==2 %Math
%             if mod(iSub,2) % In MAth narration interval the tiggers with odd order 1,3,5,7 correspond to numbers while even to operations i.e. plus
%                 trlSubUnitInfo(countsubs,23)=2;
%             end
%         end
%         countsubs=countsubs+1;
%     end
