function [mri_realigned,individual_headmodel,headshape, individual_grid_8mm, individual_grid_10mm, brain] = vy_mri_neuromag7(cfg_main)
...
%         bc = input('bias correction, yes=1, no=2:');
        bc = 2;
        if bc ==1
        ...
        %%
        sp = input('SPM set origin, yes=1, no=2:');
        if sp ==1
            ...
            % back to new ft
            restoredefaultpath
            addpath((cfg_main.allpath.spm_path));
            spm_defaults
            addpath(genpath(cfg_main.allpath.cd_org));
            setorigin_center(nii_filename_final);
            
            %% correct tilting
            set(0,'DefaultFigureWindowStyle','normal')
            % spm fmri
%             set_spm
%             spm_image('Display',nii_filename_final);
%             pause,
            close all
            ...
            individual_org = ft_read_mri(nii_filename_final);
            ft_sourceplot([], individual_org);
        end
        %%
        restoredefaultpath
        ...
        %%
        rs = input('re-slicing, yes=1, no=2:');
        if rs ==1
            cfg                 = [];
%             cfg.resolution      = 1.15;
            cfg.resolution      = 1;
...
%             cfg.dim             = [300 300 300];
        ...
        %%
        nf = input('neuromag fiducials, yes=1, no=2:');
        if nf ==1
            cfg             = [];
            cfg.interactive = 'yes';
            cfg.coordsys    = 'neuromag';
            individual_mri_neuromag   = ft_volumerealign(cfg, individual_mri_spm);
        else
            individual_mri_neuromag   = individual_mri_spm;
        end
        ...
        mri_realigned = ft_volumerealign(cfg, individual_mri_neuromag);
