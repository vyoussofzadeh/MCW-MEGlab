...
% added by VYZ
% --- Step-speed gains (defaults) ---
gChanAlign.speed.rot   = 1.0;   % rotation gain (applies to RotX/RotY/RotZ)
gChanAlign.speed.trans = 1.0;   % translation gain (TransX/TransY/TransZ)
gChanAlign.speed.scale = 1.0;   % resize gain (Resize, ResizeX/Y/Z)

% --- Lock head points (helmet-only transforms) ---
gChanAlign.lockHeadPoints = 1;  % 1=locked (default), 0=allow head-point motion

gChanAlign.hButtonLockHP = uitoggletool( ...
    hToolbar, 'CData', makeLockIcon(), ...
    'TooltipString', 'Lock head points (helmet-only transforms)', ...
    'ClickedCallback', @ToggleLockHeadPoints, ...
    'State', 'on');
...
if gChanAlign.isMeg
    gChanAlign.hButtonHelmet = uitoggletool(hToolbar, 'CData', java_geticon('ICON_DISPLAY'), 'TooltipString', 'Show/Hide MEG helmet', 'ClickedCallback', @ToggleHelmet, 'State', get(hHelmetPatch, 'Visible'));
        ...
        % - added by VYZ
    % --- Speed (step-rate) button: opens a prompt to edit gains ---
    uipushtool(hToolbar, 'CData', makeSpeedIcon(), ...
        'TooltipString', 'Set step speeds (rotation / translation / resize)', ...
        'ClickedCallback', @AdjustSpeedPrompt);
    
    uipushtool(hToolbar, 'CData', java_geticon('ICON_ROTATION_Z'), ...
        'TooltipString', 'Rotate helmet (choose axis & angle)', ...
        'ClickedCallback', @(h,e)RotateHelmetPrompt());
    
    iconAuto = makeAutoIcon();
    uipushtool(hToolbar, 'CData', iconAuto, 'separator','on', ...
        'TooltipString', 'Auto-place helmet (clean head-points + ICP)', ...
        'ClickedCallback', @(h,e)AutoPlaceHelmet());
    % ---- Optimize helmet buttons (auto-fix rotated helmet) ----
    try, iconOptYaw = java_geticon('ICON_ALIGN_CHANNELS'); catch, iconOptYaw = []; end
    if isempty(iconOptYaw), iconOptYaw = makeTargetIcon(); end
    iconOptFull = makeGearsIcon();
    
    uipushtool(hToolbar, 'CData', iconOptYaw,  'separator','on', ...
        'TooltipString', 'Optimize helmet (yaw + ICP)', ...
        'ClickedCallback', @(h,e)OptimizeHelmetPose('yaw'));
    uipushtool(hToolbar, 'CData', iconOptFull, ...
        'TooltipString', 'Optimize helmet (full 6-DoF ICP)', ...
        'ClickedCallback', @(h,e)OptimizeHelmetPose('full'));
...
function ComputeTransformation(val)
    global gChanAlign;
    if ~isfield(gChanAlign,'speed') || isempty(gChanAlign.speed)
        gChanAlign.speed.rot = 1; gChanAlign.speed.trans = 1; gChanAlign.speed.scale = 1;
    end
    ...
    switch (gChanAlign.selectedButton)
        % --- Translations (apply translation gain) ---
        case gChanAlign.hButtonTransX
            Tnew = [ (val * gChanAlign.speed.trans) / 5, 0, 0 ];
        case gChanAlign.hButtonTransY
            Tnew = [ 0, (val * gChanAlign.speed.trans) / 5, 0 ];
        case gChanAlign.hButtonTransZ
            Tnew = [ 0, 0, (val * gChanAlign.speed.trans) / 5 ];

        % --- Rotations (apply rotation gain; 'a' is in radians) ---
        case gChanAlign.hButtonRotX
            a = val * gChanAlign.speed.rot;
            Rnew = [1, 0, 0; 0, cos(a), sin(a); 0, -sin(a), cos(a)];
        case gChanAlign.hButtonRotY
            a = val * gChanAlign.speed.rot;
            Rnew = [cos(a), 0, -sin(a); 0, 1, 0; sin(a), 0, cos(a)];
        case gChanAlign.hButtonRotZ
            a = val * gChanAlign.speed.rot;
            Rnew = [cos(a), -sin(a), 0; sin(a), cos(a), 0; 0, 0, 1];

        % --- Resize (apply scale gain) ---
        case gChanAlign.hButtonResize
            Rescale = repmat(1 + val * gChanAlign.speed.scale, [1 3]);
        case gChanAlign.hButtonResizeX
            Rescale = [1 + val * gChanAlign.speed.scale, 0, 0];
        case gChanAlign.hButtonResizeY
            Rescale = [0, 1 + val * gChanAlign.speed.scale, 0];
        case gChanAlign.hButtonResizeZ
            Rescale = [0, 0, 1 + val * gChanAlign.speed.scale];

        case gChanAlign.hButtonMoveChan
            % (unchanged)
            if (length(iSelChan) ~= 1), return; end
            [p,~,vi] = select3d(gChanAlign.hSurfacePatch);
...
if ~isempty(Rnew)
    ...
    % Head points (LOCKED by default)
    if gChanAlign.isHeadPoints && ~gChanAlign.lockHeadPoints
    ...
    newTransf = eye(4); newTransf(1:3,1:3) = Rnew;
...
if ~isempty(Tnew)
    ...
    newTransf = eye(4); newTransf(1:3,4) = Tnew;
    gChanAlign.FinalTransf = newTransf * gChanAlign.FinalTransf;
end
...
switch (keyEvent.Key)   
    case {'uparrow','rightarrow'}
        step = 0.001 * keyStepMultiplier(keyEvent);   % base step * modifiers
        ComputeTransformation(step);
    case {'downarrow','leftarrow'}
        step = -0.001 * keyStepMultiplier(keyEvent);
        ComputeTransformation(step);
        % Optional quick gain shortcuts:
    case 'period'   % '.' speed up rotation
        tweakSpeed('rot', 2.0);
    case 'comma'    % ',' slow down rotation
        tweakSpeed('rot', 0.5);
    case 'slash'    % '/' speed up translation
        tweakSpeed('trans', 2.0);
    case 'quote'    % ''' slow down translation
        tweakSpeed('trans', 0.5);
    case 'a'
        AutoPlaceHelmet();
    case 'r'
        RotateHelmetPrompt();
...
% - added by VYZ
function RotateHelmetPrompt()
% Ask for axis and angle, then rotate helmet (sensors + helmet mesh).
axisStr = java_dialog('input', 'Axis (x / y / z):', ...
    'Rotate helmet', [], 'z');
if isempty(axisStr), return; end
axisChar = lower(strtrim(axisStr));
if ~ismember(axisChar, {'x','y','z'})
    bst_error('Axis must be x, y, or z.', 'Rotate helmet', 0);
    return;
end

angStr = java_dialog('input', 'Angle in degrees (e.g., -90):', ...
    'Rotate helmet', [], '-90');
if isempty(angStr), return; end
deg = str2double(strtrim(angStr));
if ~isfinite(deg)
    bst_error('Angle must be a number (degrees).', 'Rotate helmet', 0);
    return;
end

% Center of rotation: LPA/RPA midpoint by default (best for helmet)
RotateHelmetFixed(axisChar, deg, 'lpa_rpa');
end

function RotateHelmetFixed(axisChar, deg, centerMode)
% Rotate the displayed MEG helmet (sensors + helmet mesh) by a fixed angle.
% Leaves head points as-is. Updates FinalTransf so Save applies the same change.
global gChanAlign;
...
switch lower(axisChar)
    ...
    otherwise, bst_error('Axis must be x/y/z', 'Rotate helmet', 0); return;
end

O = GetRotationCenter(centerMode);       % 3x1
O = double(O(:));

% Apply to displayed sensors/helmet (row-vector form)
if ~isempty(gChanAlign.SensorsVertices)
    SV = double(gChanAlign.SensorsVertices);
    SV = (SV - O.').*1; SV = (SV * R.') + O.';  %#ok<NASGU>
    gChanAlign.SensorsVertices = (SV - O.') * R.' + O.'; % (explicit)
end
if ~isempty(gChanAlign.HelmetVertices)
    HV = double(gChanAlign.HelmetVertices);
    gChanAlign.HelmetVertices = (HV - O.') * R.' + O.';
end

% Compose homogeneous transform: x' = R*x + t, with t = O - R*O
t = O - R*O;
newTransf = eye(4);
newTransf(1:3,1:3) = R;
newTransf(1:3,4)   = t;
...
% Refresh & mark changed
UpdatePoints(1:size(gChanAlign.SensorsVertices,1));
gChanAlign.isChanged = 1;
end

function AutoPlaceHelmet()
% One-click: clean head points -> run channel_align_auto -> apply R/T to helmet
global gChanAlign;

bst_progress('start', 'Auto-place helmet', 'Preparing head points...');
% Load current ChannelMat from file
Chan = in_bst_channel(gChanAlign.ChannelFile);

% 1) Clean the head points: drop HPI + duplicate "EXTRA" labels 1..4, remove far outliers
Chan = CleanHeadPointsForICP(Chan, gChanAlign.hSurfacePatch);

% 2) Run Brainstorm's fast auto aligner (same core as "Refine with head points")
try
    [Chan2, Rnew, Tnew] = channel_align_auto(gChanAlign.ChannelFile, Chan, 1, 0);
catch ME
    bst_progress('stop');
    bst_error(['Auto-place failed: ' ME.message], 'Auto-place helmet', 0);
    return;
end
if isempty(Rnew) && isempty(Tnew)
    bst_progress('stop');
    bst_error('Auto-place did not find a better alignment. Check head points.', 'Auto-place helmet', 0);
    return;
end

% 3) Apply the transform to the displayed helmet (sensors + helmet patch)
iSelChan = 1:size(gChanAlign.SensorsVertices,1);  % all displayed
ApplyTransformation(iSelChan, Rnew, Tnew(:).', []);  % reuse existing GUI code
UpdatePoints(iSelChan);
gChanAlign.isChanged = 1;

% 4) (Optional) Report RMS distance from sensors->headpoints after fit
rmsAfter = EstimateHelmetToHeadRMS(gChanAlign);
bst_progress('stop');
if ~isempty(rmsAfter)
    fprintf('Auto-place helmet: RMS(sensor?headpoint)  %.1f mm\n', 1000*rmsAfter);
end
end

function rmsd = EstimateHelmetToHeadRMS(g)
% Rough diagnostic: RMS nearest distance from head-points(EXTRA) to displayed sensors
rmsd = [];
try
    Chan = in_bst_channel(g.ChannelFile);
    if ~isfield(Chan,'HeadPoints') || isempty(Chan.HeadPoints), return; end
    hp = Chan.HeadPoints;
    iExtra = find(strcmpi(hp.Type,'EXTRA'));
    if isempty(iExtra) || isempty(g.SensorsVertices), return; end
    X = double(hp.Loc(:,iExtra)).';        % Nx3
    Y = double(g.SensorsVertices);         % Mx3 (displayed)
    % nn distance X->Y
    d2 = zeros(size(X,1),1);
    for i=1:size(X,1)
        d2(i) = min(sum((Y - X(i,:)).^2, 2));
    end
    rmsd = sqrt(mean(d2));
catch
    rmsd = [];
...
function C = makeAutoIcon()
% 16x16 crosshair icon (fallback)
sz=16; C=ones(sz,sz,3); k=0:sz-1; [x,y]=meshgrid(k,k);
fg=[0 0 0];
C(:,8,:) = 0; C(8,:,:) = 0;
for r=[3 4 12 13]
    for ch=1:3, C(r,:,ch)=0; C(:,r,ch)=0; end
end
end

function OptimizeHelmetPose(mode)
% mode = 'yaw' (default) or 'full'
if nargin<1 || isempty(mode), mode = 'yaw'; end
global gChanAlign;
bst_progress('start','Optimize helmet', 'Cleaning head points...');

% Load channel + clean head points
Chan = in_bst_channel(gChanAlign.ChannelFile);
Chan = CleanHeadPointsForICP(Chan, gChanAlign.hSurfacePatch);
hp   = Chan.HeadPoints;
if ~isfield(hp,'Loc') || isempty(hp.Loc)
    bst_progress('stop'); bst_error('No head points available.', 'Optimize helmet', 0); return;
end
H = double(hp.Loc(:,strcmpi(hp.Type,'EXTRA'))).';   % Nx3 scalp points
if isempty(H), bst_progress('stop'); bst_error('No usable scalp points after cleaning.', 'Optimize helmet', 0); return; end

% Current helmet points (displayed)
S0 = double(gChanAlign.SensorsVertices);            % Mx3

% Center: LPA/RPA midpoint if present, else sensors centroid
O = GetRotationCenter('lpa_rpa');
if isempty(O), O = mean(S0,1).'; end

% Evaluate current RMS
rms0 = local_rms_nn(S0, H);

% Coarse yaw sweep (degrees)
thetas = -180:15:165;
best.rms = inf; best.R = eye(3); best.t = [0;0;0];

for th = thetas
    Rz = rotz_deg(th);
    S  = (S0 - O.') * Rz.' + O.';  % initial yaw guess about O
    
    % Trimmed ICP iterations from this start
    [Ricp, ticp, rms] = trimmed_icp(S, H, mode, 12, 0.15);  % 12 iters, trim top 15%
    if rms < best.rms
        best.rms = rms;
        best.R   = Ricp*Rz;                      % total from S0
        best.t   = ticp + (O - (Ricp*Rz)*O);     % compose with yaw-about-O
    end
end

% Optional fine refinement around best yaw
[yawBest] = atan2(best.R(2,1), best.R(1,1))*180/pi;
fine = (yawBest-12):3:(yawBest+12);
for th = fine
    Rz = rotz_deg(th);
    S  = (S0 - O.') * Rz.' + O.';
    [Ricp, ticp, rms] = trimmed_icp(S, H, mode, 15, 0.10);  % finer: 15 iters, trim 10%
    if rms < best.rms
        best.rms = rms; best.R = Ricp*Rz; best.t = ticp + (O - (Ricp*Rz)*O);
    end
end

% Apply best transform to displayed helmet (sensors + helmet mesh)
S1 = S0 * best.R.' + best.t.';
gChanAlign.SensorsVertices = S1;
if ~isempty(gChanAlign.HelmetVertices)
    gChanAlign.HelmetVertices = double(gChanAlign.HelmetVertices) * best.R.' + best.t.';
end
UpdatePoints(1:size(S1,1));
gChanAlign.isChanged = 1;

% Compose into FinalTransf so Save persists it
newT = eye(4); newT(1:3,1:3) = best.R; newT(1:3,4) = best.t;
gChanAlign.FinalTransf = newT * gChanAlign.FinalTransf;

bst_progress('stop');
fprintf('Optimize helmet: RMS before %.1f mm -> after %.1f mm\n', 1000*rms0, 1000*best.rms);
end

function [R, t, rms] = trimmed_icp(S, H, mode, nIter, trimFrac)
% S: Mx3 source (helmet) points (row-wise)
% H: Nx3 target (head) points
% mode: 'yaw' or 'full' rotation model
if nargin<4, nIter=12; end
if nargin<5, trimFrac=0.15; end
R = eye(3); t = zeros(3,1);
P = S;
for it=1:nIter
    % Nearest neighbors H <- P
    [idx, d2] = local_nn_idx(P, H);
    % Trim largest distances
    if trimFrac>0
        k = floor((1-trimFrac)*numel(idx));
        [~,ord] = sort(d2,'ascend'); sel = ord(1:max(k,8)); % keep at least a few
    else
        sel = 1:numel(idx);
    end
    A = P(sel,:); B = H(idx(sel),:);   % matched pairs
    
    % Solve rigid (Kabsch)
    [dR, dt] = kabsch_rigid(A, B);
    
    % If yaw-only: project dR to a pure Z-rotation
    if strcmpi(mode,'yaw')
        yaw = atan2(dR(2,1), dR(1,1));
        dR = rotz(yaw);
    end
    
    % Compose transforms (row-vector convention)
    P = (P * dR') + repmat(dt', size(P,1), 1);
    R = dR * R;
    t = dR * t + dt;
end
rms = sqrt(mean(local_nn_d2(P, H)));
end

function [R, t] = kabsch_rigid(A, B)
% Row-wise A->B rigid fit
...
function [idx, d2] = local_nn_idx(P, Q)
% for each row of P, nearest neighbor in Q
% PDIST2 may not be available everywhere; use explicit compute
nP = size(P,1); nQ = size(Q,1);
idx = zeros(nP,1); d2 = zeros(nP,1);
for i=1:nP
    D = sum((Q - P(i,:)).^2, 2);
    [d2(i), idx(i)] = min(D);
end
...
function d2 = local_nn_d2(P, Q)
% squared distance to nearest neighbor for each row of P
nP = size(P,1); d2 = zeros(nP,1);
for i=1:nP
    D = sum((Q - P(i,:)).^2, 2);
    d2(i) = min(D);
end
...
function Rz = rotz_deg(deg), th = deg*pi/180; Rz = [cos(th) -sin(th) 0; sin(th) cos(th) 0; 0 0 1]; end
function R = rotz(th), R=[cos(th) -sin(th) 0; sin(th) cos(th) 0; 0 0 1]; end

function Chan = CleanHeadPointsForICP(Chan, hSurfacePatch)
% Remove HPI + EXTRA {1..4}; drop EXTRA >10 mm from scalp surface
if ~isfield(Chan,'HeadPoints') || isempty(Chan.HeadPoints) || isempty(Chan.HeadPoints.Loc), return; end
hp = Chan.HeadPoints; N = numel(hp.Type);
isHPI = strcmpi(hp.Type,'HPI'); isExtra = strcmpi(hp.Type,'EXTRA');
isDup = false(1,N);
for k=1:N
    lab = hp.Label{k};
    if isnumeric(lab) && isscalar(lab), isDup(k) = ismember(lab,1:4);
    elseif ischar(lab) || isstring(lab), v=str2double(string(lab)); isDup(k)=~isnan(v)&&ismember(v,1:4);
    end
end
keep = ~(isHPI | (isExtra & isDup));
hp.Loc = hp.Loc(:,keep); hp.Label = hp.Label(keep); hp.Type = hp.Type(keep);
% Outlier prune
try
    if ~isempty(hSurfacePatch)
        V = get(hSurfacePatch,'Vertices'); F = get(hSurfacePatch,'Faces');
        iEx = find(strcmpi(hp.Type,'EXTRA'));
        if ~isempty(iEx)
            P = hp.Loc(:,iEx).'; D = bst_surfdist(P, V, F);    % meters
            bad = iEx(D > 0.010);   % >10 mm
            if ~isempty(bad)
                mask = true(1, numel(hp.Type)); mask(bad)=false;
                hp.Loc=hp.Loc(:,mask); hp.Label=hp.Label(mask); hp.Type=hp.Type(mask);
            end
        end
    end
catch, end
...
function O = GetRotationCenter(mode)
global gChanAlign;
O = [];
try
    Chan = in_bst_channel(gChanAlign.ChannelFile);
    hp = Chan.HeadPoints; if isempty(hp), O=[]; else
        switch lower(mode)
            case 'lpa_rpa'
                iL = find(strcmp(hp.Label,'LPA'),1);
                iR = find(strcmp(hp.Label,'RPA'),1);
                if ~isempty(iL)&&~isempty(iR), O = 0.5*(double(hp.Loc(:,iL))+double(hp.Loc(:,iR))); end
            case 'nas'
                iN = find(strcmp(hp.Label,'NAS'),1);
                if ~isempty(iN), O = double(hp.Loc(:,iN)); end
        end
    end
catch, O=[]; end
end
...
function C = makeTargetIcon()
% 16x16 target/crosshair icon
sz=16; C=ones(sz,sz,3); fg=[0 0 0];
C(:,8,:)=0; C(8,:,:)=0;
for r=[4 13], C(r,:,:) = 0; C(:,r,:)=0; end
rr=(1:sz)-8.5; [x,y]=meshgrid(rr,rr); ring=abs(sqrt(x.^2+y.^2)-5)<=0.7;
for ch=1:3, t=C(:,:,ch); t(ring)=fg(ch); C(:,:,ch)=t; end
end
function C = makeGearsIcon()
% 16x16 two-cog icon (very simple)
sz=16; C=ones(sz,sz,3);
pts=[4 4;5 4;6 5;6 6;5 7;4 7;3 6;3 5;  11 9;12 9;13 10;13 11;12 12;11 12;10 11;10 10];
for k=1:size(pts,1), C(pts(k,2),pts(k,1),:)=0; end
end


function rms = local_rms_nn(P, Q)
% Root-mean-square nearest-neighbor distance from P->Q (row-wise points)
d2 = local_nn_d2(P, Q);
rms = sqrt(mean(d2));
end

function AdjustSpeedPrompt(varargin)
    % Ask for three gains: rotation / translation / resize
    global gChanAlign;
    def = sprintf('%.3g %.3g %.3g', gChanAlign.speed.rot, gChanAlign.speed.trans, gChanAlign.speed.scale);
    answ = java_dialog('input', ...
        'Enter step speed gains:  [rotation translation resize]', ...
        'Set step speeds', [], def);
    if isempty(answ), return; end
    vals = sscanf(answ,'%f %f %f');
    if numel(vals) ~= 3 || any(~isfinite(vals))
        bst_error('Please enter three numeric values, e.g. "4 2 1".', 'Set step speeds', 0);
        return
    end
    vals = max(vals, 1e-4);   % clamp to sensible min
    gChanAlign.speed.rot   = vals(1);
    gChanAlign.speed.trans = vals(2);
    gChanAlign.speed.scale = vals(3);
    fprintf('Step speeds set: rot x%.3g, trans x%.3g, scale x%.3g\n', vals(1), vals(2), vals(3));
end

function C = makeSpeedIcon()
% 16x16 icon: ">>" arrows (meaning faster)
sz=16; C=ones(sz,sz,3);
pts = [5 5; 6 6; 7 7; 6 8; 5 9;  9 5; 10 6; 11 7; 10 8; 9 9];
for k=1:size(pts,1), C(pts(k,2),pts(k,1),:)=0; end
end


function m = keyStepMultiplier(keyEvent)
    % SHIFT = 10x, CTRL = 0.1x, ALT = 0.01x (combine multiplicatively)
    m = 1;
    if isfield(keyEvent,'Modifier') && ~isempty(keyEvent.Modifier)
        if any(strcmpi(keyEvent.Modifier,'shift')),   m = m * 10;  end
        if any(strcmpi(keyEvent.Modifier,'control')), m = m * 0.1; end
        if any(strcmpi(keyEvent.Modifier,'alt')),     m = m * 0.01;end
    end
end

function tweakSpeed(which, factor)
    global gChanAlign;
    switch which
        case 'rot',   gChanAlign.speed.rot   = max(1e-4, gChanAlign.speed.rot   * factor);
        case 'trans', gChanAlign.speed.trans = max(1e-4, gChanAlign.speed.trans * factor);
        case 'scale', gChanAlign.speed.scale = max(1e-4, gChanAlign.speed.scale * factor);
    end
    disp(sprintf('Speed updated: rot x%.3g | trans x%.3g | scale x%.3g', ...
 gChanAlign.speed.rot, gChanAlign.speed.trans, gChanAlign.speed.scale));
end


function C = makeLockIcon()
% 16x16 black lock on white background
sz = 16;
C  = ones(sz,sz,3);          % white background
% shackle (a U shape)
C(4,6:10,:) = 0;             % top bar
C(5:6,6,:)  = 0;             % left leg
C(5:6,10,:) = 0;             % right leg
% body (rectangle)
C(7:12,5:11,:) = 0;          % lock body
% keyhole (white vertical slit)
C(9:11,8,:) = 1;
end

function ToggleLockHeadPoints(~,~)
    global gChanAlign;
    isOn = strcmpi(get(gChanAlign.hButtonLockHP,'State'),'on');
    gChanAlign.lockHeadPoints = isOn;
    % (Optional) swap icon for unlocked state:
    % set(gChanAlign.hButtonLockHP, 'CData', tern(isOn, makeLockIcon(), makeUnlockIcon()));
end
% (Optional) tiny ternary helper:
