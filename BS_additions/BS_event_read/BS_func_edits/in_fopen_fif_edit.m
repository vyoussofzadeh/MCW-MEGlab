function [sFile, ChannelMat] = in_fopen_fif_edit(DataFile, ImportOptions, nLink)
...
elseif (nEpochs == 0)
    ...
    if iscell(ImportOptions.EventsMode) || ~strcmpi(ImportOptions.EventsMode, 'ignore')
        [sFile.events, ImportOptions] = fif_read_events_edit(sFile, ChannelMat, ImportOptions);
