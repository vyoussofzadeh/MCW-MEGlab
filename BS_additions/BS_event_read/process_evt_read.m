...
%% Added by VY on 1/17/24
% for iBlock = 1:nbBlocks
    
    % Store events from original tracks
    eventsOriginal = events;
    
    % Create normalized tracks
    normalizedTracks = (tracks_vals{1} - min(tracks_vals{1})); % Normalize
    normalizedTracks = bitand(normalizedTracks, 255); % Mask
    
    % Process normalized tracks
    % You can either call a separate function or replicate the logic here
    eventsNormalized = ProcessTriggers(normalizedTracks, sFile, StimChan, EventsTrackMode, isAcceptZero, MinDuration, samplesBlock);
    % Label these events as 'Normalized'
    for i = 1:length(eventsNormalized)
        eventsNormalized(i).label = ['Normalized_' eventsNormalized(i).label];
    end
    
    % Merge events from original and normalized tracks
    events = [eventsOriginal, eventsNormalized];
...
function events = ProcessTriggers(tracks, sFile, StimChan, EventsTrackMode, isAcceptZero, MinDuration, samplesBlock)

% @author: Vahab Youssofzadeh
