...
%%
ep = []; cfg = [];
cfg.toilim = PostStim; ep.pst = ft_redefinetrial(cfg, ftData);
cfg.toilim = Baseline; ep.bsl = ft_redefinetrial(cfg, ftData);
...
%%
ep.app = ft_appenddata([], ep.pst, ep.bsl);
tlk = do_timelock(ep);
...
cfg.lcmv.fixedori    = 'yes'; % project on axis of most variance using SVD
cfg.lcmv.lambda      = '0.1%';
s_data.bsl = ft_sourceanalysis(cfg, tlk.bsl);
...
conn_bsl = do_conn(s_data.bsl, Connres);
...
if filt_ask ==1
    ResultsMat.Comment       = ['ft_connanalysis: CrossSpectral_' Method, '_', num2str(foi(1)), '-', num2str(foi(2)), 'Hz-', datestr(now, 'dd/mm/yy-HH:MM')];
else
...
function [outsum] = do_conn(source, Connres)
