...
num = [];
for j=1:length(sInput)
    [~, name, ~] = fileparts(sInput(j).DataFile);
    idx = strfind(name, 'trial');    
    str = (name(idx+4:end));
    num_str = regexp(str, '\d+', 'match'); % Extract numeric part of the string
    num(j) = str2double(num_str); % Convert the numeric string to a double
end
...
DipoleFiles = DipoleFiles(unique(num));
disp(DipoleFiles')
