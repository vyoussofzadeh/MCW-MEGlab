function varargout = process_ft_sourceanalysis_DICS_BF_rest(varargin )
...
sProcess.Comment     = 'FieldTrip: ft_sourceanalysis DICS-BF (rest) v010620';
...
for iChanFile = 1:1%length(AllChannelFiles)
    ...
    for iInput = 1:1%length(iChanInputs)
        ...
        TI = ftData1.time{2}(1) - ftData1.time{1}(1);
        ftData1.time{1}(end) - ftData1.time{1}(1);
        ...
        if exist(fullfile(savepath,'tfr.mat'),'file')~= 2
            ...
            if TI ~= 0, tfr.time = linspace(0, ftData1.time{1}(end) - ftData1.time{1}(1), length(cfg.toi)); end
        ...
        for i=1:length(datain.time)
            datain.time{i} = linspace(0, ftData1.time{1}(end) - ftData1.time{1}(1), length(ftData1.time{1}));
        end
        
        warning(['Maximum trial length:[', num2str(datain.time{1}(1)), ',', num2str(datain.time{1}(end)),']']);
        disp('Enter time interval in sec, e.g., [0.7,1.2];');
        toi = input(':');
       ...
       %% step7: spectral analysis
        ...
        disp(['Suggested by TFR: ', num2str(f_sugg)]);
        disp('Select foi:')
        clear('input')
        f = input('Freq of interest? ');
        
        disp('Select tappering freq:')
        tapsmofrq = input(':');
%         tapsmofrq = 4;
...
%                 source_diff_dics.pow(source_diff_dics.pow<0)=0;
        ...
        ResultsMat.Comment       = ['ft_sourceanalysis_rest: ' Method, '_',num2str(f),'Hz_', num2str(toi(1,1)),'_',num2str(toi(1,2)), 'sec ', datestr(now, 'dd/mm/yy-HH:MM')];
