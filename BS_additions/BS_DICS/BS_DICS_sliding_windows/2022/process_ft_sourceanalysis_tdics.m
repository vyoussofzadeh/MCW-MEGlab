function varargout = process_ft_sourceanalysis_tdics(varargin )
...
sProcess.Comment     = 'FieldTrip: ft_sourceanalysis (time-DICS)';
...
cfg.latency   = [Baseline(1)+bf,PostStim(2)-bf]; %[0,PostStim(2)];
...
for time_intervals = 1:length(winStarts)
    ...
    cfg.dics.fixedori    = 'yes'; % project on axis of most variance using SVD
    cfg.headmodel = ftHeadmodel;
    cfg.latency   = winStarts(time_intervals,:);
    ...
    source_diff_dics.pow = (s_data.pst.pow - s_data.bsl.pow)./(s_data.pst.pow + s_data.bsl.pow);
    source_diff_dics.pow(source_diff_dics.pow>0)=0;
    source_diff_dics.pow = source_diff_dics.pow';
    
    pow(time_intervals,:) = source_diff_dics.pow;
end

%%
source_diff_dics.pow = pow';
...
%     disp([start_idx, end_idx])
    ...
    output_matrix(:, start_idx:end_idx) = pow(i,:)' + output_matrix(:, start_idx:end_idx);
end

%%
source_diff_dics.pow = output_matrix;
...
ResultsMat.Time          = DataMat.Time;%linspace(winStarts(1,1), winStarts(end,1), length(DataMat.Time));
