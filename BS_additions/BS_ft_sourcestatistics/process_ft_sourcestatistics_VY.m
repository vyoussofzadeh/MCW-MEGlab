function varargout = process_ft_sourcestatistics_VY( varargin )
...
sProcess.Comment     = 'FieldTrip: ft_sourcestatistics VY';
...
switch (OPT.StatisticType)
    case 'indepsamplesT'
        ftAllFiles_null = ftAllFiles;
        tmp = zeros(size(ftAllFiles_null{1}.pow,1),1);
        for i=1:length(ftAllFiles_null)
            ftAllFiles_null{i}.pow = tmp;
        end
        ftAllFiles = [ftAllFiles,ftAllFiles_null];
end

%% Edits to corrent for atlas-based analysis
for i=1:length(ftAllFiles)
    ftAllFiles{i}.pow = ftAllFiles{i}.pow(:,1);
end
...
switch (OPT.StatisticType)
    case 'indepsamplesT'
        statcfg.design      = zeros(1, 1*nFilesA); %zeros(1, nFilesA + nFilesB);
        statcfg.design(1, 1:nFilesA)             = 1;
        statcfg.design(1, nFilesA + 1:nFilesA*2) = 2;
    ...
    case 'depsamplesT'
        ...
        statcfg.ivar        = 1;   % the 2nd row in cfg.design contains the independent variable
        statcfg.uvar        = 2;   % the 1st row in cfg.design contains the subject number (or trial number)
end
...
%% Stat-thresholding
disp('Stat threshold, eg, 0.5 ');
thre = input(':');
% thre = 0.5;
stat = ftStat.stat;
stat (abs(stat) < thre.*max(abs(stat))) = nan;
ftStat.stat = stat;
