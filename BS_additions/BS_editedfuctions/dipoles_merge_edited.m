function OutputFile = dipoles_merge_edited(DipoleFiles)
...
for i = 1:length(DipoleFiles)
    ...
    nSubsets = 5;
    for g = 1:nGroups/nSubsets
        
        for j=1:nSubsets
            % Add the name
            MergeMat.DipoleNames{end+1} = ['Group #', num2str(g+lastGroupNumber/nSubsets), ' (', num2str(j), ')'];
        end
...
MergeMat.Comment = ['Merge: ' num2str(length(DipoleFiles)) ' files (5 subsets)'];
