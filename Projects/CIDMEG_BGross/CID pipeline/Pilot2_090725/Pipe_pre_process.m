...
datadir = '/group/bgross/work/CIDMEG/ECOG_MEG_data/MEG/mcwa086_v1';
...
%% MEGnet-ICA analysis - Part 3 data
cd(datadir)
...
BS_dir = '/group/bgross/work/CIDMEG/Analysis/BrainstormProcess/Brainstorm_db/Pilot2_092025';
...
for ii = 1:L_data
    ...
    if  isempty(dir(fullfile(['./@rawica_',runNumber, '_clean'],'/channel_*.mat'))) && ...
        ...
        disp(RawFiles)
        pause,
        if Group_analysis_exist, iSubject = iSubject+1; end
...
for i=1:length(d)
    ...
    if ~exist(newFolderName, 'dir')
        ...
        switch eventname
            case '8'
                newFolderName = fullfile(pathstr2, [subjectID, '_', runNumber,'_resp']);
            case '1'
...
for i=1:length(d)
    ...
    elseif contains(pathstr, 'mcwa065_v1')
        dtag = 'STI101';
    elseif contains(pathstr, 'mcwa086_v1')
        dtag = '2';
    end
    ...
    if contains(pathstr, 'mcwa086_v1')
        if ~isempty(dd) && flag == 1
            dtag = '8';
