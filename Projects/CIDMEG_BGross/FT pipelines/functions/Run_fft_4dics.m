...
% f_data.bsl = do_fft(ep_data.bsl, [2,40], 0,[],0); f_data.bsl.elec = sens;
% f_data.pst = do_fft(ep_data.pst, [2,40], 0,[],0); f_data.pst.elec = sens;
...
disp(['Suggested by TFR: ', num2str(f_sugg),'Hz']);
%     if f_sugg > 12
%         f = f_sugg;
%     else
f = input('FOI (Hz)? '); 
...
[f_data.app,~,~,tapsmofrq] = do_fft(cfg, ep_data.app); f_data.app.elec = cfg_main.sens;
f_data.bsl = do_fft(cfg, ep_data.bsl); f_data.bsl.elec = cfg_main.sens;
f_data.pst = do_fft(cfg, ep_data.pst); f_data.pst.elec = cfg_main.sens;

% f = 20;
% % Freq of interest - prepration for DICS source analysis
% f_data.app = do_fft(ep_data.app, [f,f], 0,[],0); f_data.app.elec = sens;
% f_data.bsl = do_fft(ep_data.bsl, [f,f], 0,[],0); f_data.bsl.elec = sens;
% f_data.pst = do_fft(ep_data.pst, [f,f], 0,[],0); f_data.pst.elec = sens;