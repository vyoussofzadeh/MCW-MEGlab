...
for bb = 1:5
    close all;
    datafile = fullfile(indir,['Tones_Block', num2str(bb), '_raw.fif']);
...
cd(outdir)
save(['dataclean_tone_', subj], 'data_clean', '-v7.3');
%
%% Combine all blocks
...
alldata.sampleinfo = cat(1, data_clean{1}.sampleinfo, ...
    ...
    data_clean{5}.sampleinfo ...    %data_clean{6}.sampleinfo
    );
...
alldata.trialinfo = cat(1, data_clean{1}.trialinfo, ...
    ...
    data_clean{5}.trialinfo ...     %data_clean{6}.trialinfo ...
    );
...
alldata.trial = cat(2, data_clean{1}.trial, ...
    ...
    data_clean{3}.trial, ...
    data_clean{4}.trial, ...
    data_clean{5}.trial ...    %data_clean{6}.trial ...
    );
...
alldata.time = cat(2, data_clean{1}.time, ...
    ...
    data_clean{3}.time, ...
    data_clean{4}.time, ...
    data_clean{5}.time ...    %data_clean{6}.time ...
    )
save(['all_dataclean_tone_', subj], 'alldata', '-v7.3');
