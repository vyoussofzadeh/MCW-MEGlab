function run_spike_classifier_withTESTdata_2(varargin)
...
% Build spike first to capture reference channel labels, then align nospike
% [dataSpike, groupsS, refLbl]  = build_cells(filesS, 1, @extract_group_id);
% [dataNoSpike, groupsN]        = build_cells_with_ref(filesN, refLbl, @extract_group_id);
%
...
%
% % Sanity: enforce identical channel counts/order across all epochs
% uChTmp = unique(cellfun(@(x) size(x,1), [Dnspk; Dspk]));
% assert(numel(uChTmp)==1, 'Channel mismatch across epochs after alignment. Check label consistency.');
%
% data   = [Dnspk; Dspk];
...
% Ensure training set is non-empty after grouped split
assert(~isempty(XTrain), 'Training set is empty after group split. Reduce valFrac or check group IDs.');

%%
% Ensure consistent class order across Train/Val
cats = categories(YTrain);
YTrain = reordercats(YTrain, cats);
YVal   = reordercats(YVal, cats);

% Compute class weights from ORIGINAL YTrain (pre-oversample!)
cnt  = countcats(YTrain);
w    = sum(cnt)./max(cnt,1);   % inverse frequency
w    = w / mean(w);            % normalize around 1.0

% Build a weighted classification layer youll reuse below
...
% Augmentations (training only)  cropping, sensor-shift, phase flip, freq-mix
if opts.augment
    % Negative pool for frequency-mix (pull from current normalized training set)
    negPool = XTrain2(YTrain=='NoSpike');
    % Channel info (optional, for label-aware shifts) derived from reference labels
    chanInfo = struct();
    if exist('refLbl','var') && ~isempty(refLbl)
        isMag  = endsWith(refLbl,'1');
        isGrad = endsWith(refLbl,'2') | endsWith(refLbl,'3');
        chanInfo.idxMag  = find(isMag);
        chanInfo.idxGrad = find(isGrad);
    end
    if ~isfield(opts,'aug') || isempty(opts.aug)
        aug = default_meg_aug();
    else
        aug = fill_default_meg_aug(opts.aug);
    end
    A = make_meg_augmentor(aug, negPool, chanInfo);
    XTrain2 = cellfun(@(x) A(x), XTrain2,'UniformOutput',false);
end
...
useOversample = false;  % < turn on OR use class weights, not both
if useOversample
...
switch lower(opts.arch)
    case 'resnet1d'
        ...
        % Build the layers (channels = size along rows)
        lgraph = resnet1d_layers(inputSize, 2);  % 2 classes: NoSpike/Spike
        ...
        trainOpts = trainingOptions('adam', ...
            ...
            'GradientThreshold',1, ...
            padArgs{:}, ...                        % "SequenceLength","longest"
            'Shuffle','every-epoch', ...
    ...
    case 'spatiotemporal_dualbranch'

        % After making XTrain/YTrain, and BEFORE oversampling:
        cats = categories(YTrain);
        ...
        % Build model
        inputSize = size(XTrain{1},1);   % channels
        lg = spatiotemporal_dualbranch_layers(inputSize, 2, clsLayer);

        % Training options (cap sequence length if needed)
        maxLen = prctile(cellfun(@(x) size(x,2), XTrain), 95);
        trainOpts = trainingOptions('adam', ...
            'InitialLearnRate',3e-4, 'MaxEpochs',120, 'MiniBatchSize',128, ...
            'Shuffle','every-epoch', 'ValidationData',{XVal2,YVal}, ...
            'ValidationFrequency', max(5,ceil(numel(XTrain)/128)), ...
    ...
    case 'spatiotemp_2d_cnn'

        hdr = ft_read_header('/data/MEG/Research/SpikeDetection/tsss_ecgClean_Data/alioto_courtney_Run03_spont_supine_raw_t_sss_ecgClean_raw.fif');  % or ft_read_sens
        grad = hdr.grad;                                      % Neuromag grad struct
        % 2D plot coords:
        cfg = []; cfg.grad = grad; cfg.layout = 'neuromag306all.lay';
        lay = ft_prepare_layout(cfg);
        % Map lay.label to your refLbl as above:

        labAll = hdr.label;                          % all channels
        megMask = startsWith(labAll,'MEG');
        refLbl  = labAll(megMask);
        posXY = normalize(lay.pos(:,1:2), "range", [-1 1]);
        % posXY = posXY(matchLabels(lay.label, refLbl), :);     % write a small matcher

        % refLbl must match your row order in X{...}
        posXY = get_posXY_from_labels(refLbl);   % [C x 2]

        % 1) Decide a fixed T across train+val
        Ttrain = cellfun(@(x) size(x,2), XTrain2);
        Tval   = cellfun(@(x) size(x,2), XVal2);
        Tfix   = max(1, round(prctile([Ttrain(:); Tval(:)], 95)));   % e.g., 95th pct

        % 2) Pad/crop BOTH sets to Tfix
        XTrain2 = pad_sequences_to_length(XTrain2, Tfix);
        XVal2   = pad_sequences_to_length(XVal2,   Tfix);

        % 3) Build maps for Tfix and convert to 4D batches
        maps   = buildSpatialMaps(posXY, Tfix);  % [C x Tfix x 2]
        XtrImg = toImageBatch(XTrain2, maps);    % [C x Tfix x 3 x Ntr]
        XvaImg = toImageBatch(XVal2,   maps);    % [C x Tfix x 3 x Nval]

        numInCh = size(XtrImg,3);  % 3 if maps added

        C = size(XtrImg,1);  T = size(XtrImg,2);
        ...
        lg = spatiotemporal_cnn_layers_fast(C, T, numInCh, 2, clsLayer);  % from earlier


        % (Optional) Decimate time dimension first:
        % XtrImg = decimate_time_2x_4d(XtrImg);  XvaImg = decimate_time_2x_4d(XvaImg);

        mb = 128;  % try 256 if GPU RAM allows
        valFreq = max(20, ceil(size(XtrImg,4)/mb));  % validate less often

        opts = trainingOptions('adam', ...
            'InitialLearnRate',3e-4, ...
            'LearnRateSchedule','piecewise','LearnRateDropPeriod',20,'LearnRateDropFactor',0.5, ...
            'L2Regularization',5e-4, ...
            'MaxEpochs',60, 'MiniBatchSize',mb, ...
            'Shuffle','every-epoch', ...
            'ValidationData',{XvaImg, YVal}, 'ValidationFrequency',valFreq, ...
            'ValidationPatience',8, ...
            'ExecutionEnvironment','auto', ...  % force GPU if you have one
            'Plots','none', 'Verbose',true);

        net = trainNetwork(XtrImg, YTrain, lg, opts);
    ...
    case 'lstm'
        ...
        layers = [
            ...
            softmaxLayer
            classificationLayer("Name","cls")];   % keep the name "cls"
        % classificationLayer];
    ...
    case 'residual tcn' %  (multi-kernel, dilated)

        % After you have XTrain2, YTrain, XVal2, YVal (cells of CxT), normalized
        % Optional: cap sequence length to stabilize memory
        maxLen = max(1, round(prctile(cellfun(@(x) size(x,2), XTrain2), 95)));
        padArgs = {"SequenceLength", maxLen, "SequencePaddingDirection","right"};

        % Class weights (or skip if you oversampledont do both)
        cats = categories(YTrain); YTrain = reordercats(YTrain,cats); YVal = reordercats(YVal,cats);
        cnt = countcats(YTrain);  w = sum(cnt)./max(cnt,1);  w = w/mean(w);
        clsLayer = classificationLayer('Name','cls','Classes',cats,'ClassWeights',w);

        % Build TCN
        inputSize = size(XTrain2{1},1);  % channels
        params = struct('stemWidth',64,'blockWidths',[64 64 96 128], ...
            'kernels',[3 7 15],'dilations',[1 2 4 8], ...
            'dropout',0.3,'useGAPGMP',true);
        lg = spiketcn_layers(inputSize, 2, params, clsLayer);

        % Train
        mb = 128; valFreq = max(5, ceil(numel(XTrain2)/mb));
        opts = trainingOptions('adam', ...
            'InitialLearnRate',3e-4, ...
            'LearnRateSchedule','piecewise','LearnRateDropPeriod',30,'LearnRateDropFactor',0.5, ...
            'L2Regularization',5e-4, ...
            'MaxEpochs',120, 'MiniBatchSize',mb, ...
            'Shuffle','every-epoch', ...
            padArgs{:}, ...
            'ValidationData',{XVal2,YVal}, 'ValidationFrequency',valFreq, ...
            'ValidationPatience',12, ...
            'GradientThresholdMethod','l2norm','GradientThreshold',1, ...
            'ExecutionEnvironment','auto','Plots','training-progress');

        net = trainNetwork(XTrain2, YTrain, lg, opts);
...
order = categories(YVal);         % {'NoSpike','Spike'}
cm = confusionmat(YVal, YP, 'Order', order);
acc = sum(diag(cm))/sum(cm,'all');
rec = diag(cm) ./ max(1,sum(cm,2));  % sensitivity/TPR per class
prec = diag(cm) ./ max(1,sum(cm,1)');% precision per class
f1   = 2*(prec.*rec) ./ max(prec+rec,eps);
metrics = table(order, prec, rec, f1, 'VariableNames', {'Class','Precision','Recall','F1'});
disp(metrics)
balAcc = mean(rec,'omitnan');
...
try
    [st,hash] = system(sprintf('git -C "%s" rev-parse HEAD', opts.codeRoot));
    git_hash = ternary(st==0, strtrim(hash), 'NA');
catch
    git_hash = 'NA';
end
meta = struct( ...
...
function setup_environment(o)
addpath(o.ftRoot); ft_defaults;
addpath(genpath(fullfile(o.codeRoot,'FT_functions','functions_new')));  % fixed
addpath(genpath(fullfile(o.codeRoot,'FT_functions','helper')));         % fixed
end
