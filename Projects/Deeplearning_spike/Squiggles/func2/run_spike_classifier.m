...
cv = cvpartition(labels,'HoldOut',opts.valFrac);
...
switch lower(opts.arch)
    case 'lstm2'
        ...
        w          = max(counts) ./ counts;              % inverse-frequency

        layers = [
            sequenceInputLayer(inputSize,'Normalization','none')
            bilstmLayer(128,'OutputMode','sequence')
            dropoutLayer(0.3)                             % a bit more dropout
            bilstmLayer(64,'OutputMode','last')
            dropoutLayer(0.3)
            fullyConnectedLayer(2)
            softmaxLayer
            classificationLayer('Classes',classNames,'ClassWeights',double(w'))
            ];

        trainOpts = trainingOptions('adam', ...
            'InitialLearnRate',3e-4, ...                  % smaller LR
            'L2Regularization',1e-4, ...                  % weight decay
            'MaxEpochs',150, ...
            'MiniBatchSize',128, ...
            'SequenceLength','shortest', ...              % avoid long padding
            'Shuffle','every-epoch', ...
        ...
        mdlName = 'lstm2_spike_classifier.mat';
