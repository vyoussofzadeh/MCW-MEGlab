function run_spike_classifier4(varargin)
...
% % main: Conv1D(k,filters,dil) -> BN -> ReLU -> Dropout -> Conv1D(k,filters) -> BN
% % skip: identity or 1x1 Conv1D to match channels
% % output: Add -> ReLU
% 
% % Main path layers
% main = [
%     convolution1dLayer(k, filters, 'Padding','same', 'DilationFactor',dil, 'Name',[blk '_conv1'], 'WeightsInitializer','he')
%     batchNormalizationLayer('Name',[blk '_bn1'])
%     reluLayer('Name',[blk '_relu1'])
%     dropoutLayer(dropP,'Name',[blk '_drop'])
...
% lgraph = addLayers(lgraph, main);
% 
% % Skip path: projection if channel count changes
% if inCh ~= filters
%     skip = [
%         convolution1dLayer(1, filters, 'Padding','same', 'Name',[blk '_proj'], 'WeightsInitializer','he')
%         batchNormalizationLayer('Name',[blk '_bnskip'])];
%     lgraph = addLayers(lgraph, skip);
%     skipOut = [blk '_bnskip'];
%     % wire skip
%     lgraph = connectLayers(lgraph, inName, [blk '_proj']);
%     lgraph = connectLayers(lgraph, [blk '_proj'], [blk '_bnskip']);
% else
%     skipOut = inName; % identity
% end
% 
% % Addition and output ReLU
% addL = additionLayer(2,'Name',[blk '_add']);
% relu2 = reluLayer('Name',[blk '_relu2']);
% lgraph = addLayers(lgraph, addL);
% lgraph = addLayers(lgraph, relu2);
...
% % Wire main path
% lgraph = connectLayers(lgraph, inName, [blk '_conv1']);
% lgraph = connectLayers(lgraph, [blk '_conv1'], [blk '_bn1']);
% lgraph = connectLayers(lgraph, [blk '_bn1'], [blk '_relu1']);
% lgraph = connectLayers(lgraph, [blk '_relu1'], [blk '_drop']);
% lgraph = connectLayers(lgraph, [blk '_drop'], [blk '_conv2']);
% lgraph = connectLayers(lgraph, [blk '_conv2'], [blk '_bn2']);
% lgraph = connectLayers(lgraph, [blk '_bn2'], [blk '_add/in1']);
% 
% % Wire skip into add/in2
% lgraph = connectLayers(lgraph, skipOut, [blk '_add/in2']);
% 
% % Wire to output ReLU
% lgraph = connectLayers(lgraph, [blk '_add'], [blk '_relu2']);
