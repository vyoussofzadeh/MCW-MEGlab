function run_spike_classifier_withTESTdata8(varargin)
...
data = [];
data.label    = refLbl(:);
data.fsample  = opts.fs;
data.trial    = cell(N,1);
data.time     = cell(N,1);
for i = 1:N
    Xi = Xtr2{i};
    assert(isequal(size(Xi),[C T]), 'Epoch %d size mismatch', i);
    data.trial{i} = Xi;                 % C×T
    data.time{i}  = t;                  % 1×T
end

% Attach geometry (recommended for MEG topoplots/forward ops)
if exist('sens','var') && ~isempty(sens)
    data.grad = sens;                   % for MEG
end

cfg = [];
cfg.keeptrials = 'yes';
timelock_trials = ft_timelockanalysis(cfg, data);   % .trial: N×C×T, .time, carries grad

%% ===== Timelock VAL =====
Nva = numel(Xva2); C = size(Xva2{1},1); T = size(Xva2{1},2);
t = times;

dataVa = [];
dataVa.label   = refLbl(:);
dataVa.fsample = opts.fs;
dataVa.trial   = cell(Nva,1);
dataVa.time    = cell(Nva,1);
for i=1:Nva
    dataVa.trial{i} = Xva2{i};
    dataVa.time{i}  = t;
end
cfg = []; cfg.keeptrials = 'yes';
timelock_trials_tr = timelock_trials;    % rename train
timelock_trials_va = ft_timelockanalysis(cfg, dataVa);

%%
cfg = [];
cfg.latency = [-50 50];   % seconds (not ms)
timelock_trials_tr_sel = ft_selectdata(cfg, timelock_trials_tr);
timelock_trials_va_sel = ft_selectdata(cfg, timelock_trials_va);

timelock_trials_tr_sel = timelock_trials_tr;
timelock_trials_va_sel = timelock_trials_va;

%%
[ImTr, ImVa, meta] = timelock_to_4d(timelock_trials_tr_sel, timelock_trials_va_sel, ...
                                    'Depth4', false, 'Normalize', false);
C = meta.C; T = meta.T; D = meta.D;

%% ===== Labels (reuse your split to avoid leakage) =====
cats = categories(YTrain);
YTr  = categorical(YTrain, cats);
YVa  = categorical(YVal,   cats);

%% ===== Per-channel normalization (fit on TRAIN only) =====
Xall = reshape(ImTr, C, []);            % collapse (T*D*batch)
mu = mean(Xall,2); sd = std(Xall,0,2);  sd(sd<1e-12)=1;
for i=1:size(ImTr,4), for d=1:D, ImTr(:,:,d,i) = (ImTr(:,:,d,i) - mu) ./ sd; end, end
for i=1:size(ImVa,4), for d=1:D, ImVa(:,:,d,i) = (ImVa(:,:,d,i) - mu) ./ sd; end, end
...
cfg = [];
cfg.layout  = lay;            % from A or B
cfg.marker  = 'on';
cfg.zlim    = 'maxabs';
cfg.xlim    = [-25 25];   % average 80140 ms
figure('Color','w');
ft_topoplotER(cfg, timelock_trials);
ft_topoplotER(cfg, timelock_trials_tr);
title('Topomap (-20 to 20 ms)');

%%
% Convert to sequences for 1D models
[Xtr_tem, Xva_tem, inputSize_tem, Tsmall] = timelock_to_seq1d(timelock_trials_tr_sel, timelock_trials_va_sel, ...
                                                              'UsePCA', false, 'VarKeep', 98);
...
if opts.trainCNN2D
    ...
    %% ===== 2D CNN (one layer per row; no commas) =====
    layers2d = [
    ...
    % If you still hit vertcat error, remove the commas after each layer:
    %   convolution2dLayer(...)\n batchNormalizationLayer\n reluLayer\n ...

    valFreq = max(10, ceil(size(ImTr,4)/128));
    opts2d = trainingOptions('adam', ...
        ...
        'Shuffle','every-epoch', ...
        'ValidationData',{ImVa,YVa}, ...
        'ValidationFrequency',valFreq, ...
        'ValidationPatience',8, ...
        'ExecutionEnvironment',execEnv, ...
        'Plots','training-progress','Verbose',true);

    models.net_2d = trainNetwork(ImTr, YTr, layers2d, opts2d);

    % Evaluate
    [Yhat2d, s2d] = classify(models.net_2d, ImVa);
    cm2d = confusionmat(YVa, Yhat2d, 'Order', cats);
    acc2d = sum(diag(cm2d))/sum(cm2d,'all');
    rec2d = diag(cm2d)./max(1,sum(cm2d,2));
    BA2d  = mean(rec2d);
    fprintf('[CNN2D] Acc=%.3f BA=%.3f\n', acc2d, BA2d);
    models.cnn2d_val = struct('acc',acc2d,'BA',BA2d,'cm',cm2d,'scores',s2d,'ImVa',ImVa);
