function run_spike_classifier_withTESTdata5(varargin)
...
switch lower(opts.arch)
    ...
    case 'mamba_proxy'
        % Assume XTrain2, XVal2: cell of [C×T] (normalized); YTrain, YVal: categorical
        % (Optional) decimate to cut time:
        decim = 2;  % try 2 or 4
        XTr = cellfun(@(x) single(x(:,1:decim:end)), XTrain2,'uni',false);
        XVa = cellfun(@(x) single(x(:,1:decim:end)), XVal2,  'uni',false);

        % Tight sequence length (shorter = faster)
        maxLen = round(prctile(cellfun(@(x) size(x,2), XTr), 70));

        % Sizes & class weights
        inputSize = size(XTr{1},1);
        cats = categories(YTrain);
        YTrain = reordercats(YTrain, cats);  YVal = reordercats(YVal, cats);
        cnt = countcats(YTrain); w = sum(cnt)./max(cnt,1); w = w/mean(w);
        clsLayer = classificationLayer('Name','cls','Classes',cats,'ClassWeights',w);

        % Build Mamba-proxy graph
        lg = mamba_proxy_layers(inputSize, numel(cats), clsLayer);

        % Train (fast settings)
        opts = trainingOptions('sgdm', ...
            'InitialLearnRate',5e-10, 'Momentum',0.9, ...
            'MaxEpochs',120, 'MiniBatchSize',256, ...
            'Shuffle','every-epoch', ...
            'SequenceLength',maxLen, 'SequencePaddingDirection','right', ...
            'GradientThresholdMethod','l2norm','GradientThreshold',1, ...
            'ValidationData',{XVa, YVal}, ...
            'ValidationFrequency', max(20, ceil(numel(XTr)/256)), ...
            'ValidationPatience',5, ...
            'Plots','training-progress', 'ExecutionEnvironment','auto', 'Verbose',false);

        net = trainNetwork(XTr, YTrain, lg, opts);

    case 'lv_cadenet'

        plot_lv_channel(XTrain{1, 1}, fs)        % auto-picks a channel
% or
plot_lv_channel(Xepoch, fs, 123)   % plot channel 123


plot_lv_heatmaps(XTrain{1, 1}, fs)

plot_lv_overview(XTrain{1, 1}, fs)

plot_stacked_vertical(XTrain{1, 1}, fs)         % all channels

plot_rows_tiled(XTrain{1, 1}, fs)

ft_view_matrix(XTrain{7, 1}, fs);   % refLbl = your channel names


        % (1) optional decimate to cut time in half/quarter
decim = 2;                           % try 2 or 4
XTr = cellfun(@(x) single(x(:,1:decim:end)), XTrain2,'uni',false);
XVa = cellfun(@(x) single(x(:,1:decim:end)),   XVal2,'uni',false);

% (2) short padded length (keeps batches fast)
maxLen = round(prctile(cellfun(@(x) size(x,2), XTr), 70));

% (3) tiny Mamba-proxy / BiLSTM
inputSize = size(XTr{1},1);
cats = categories(YTrain);
YTrain = reordercats(YTrain,cats); YVal = reordercats(YVal,cats);
cnt = countcats(YTrain); w = sum(cnt)./max(cnt,1); w = w/mean(w);
...
layers = [
  ...
  bilstmLayer(32,'OutputMode','last')    % or gruLayer(32)
  dropoutLayer(0.2)
...
opts = trainingOptions('sgdm', ...
  ...
  'MaxEpochs',20,'MiniBatchSize',256, ...
  'SequenceLength',maxLen,'SequencePaddingDirection','right', ...
  'ValidationData',{XVa,YVal}, ...
  'ValidationFrequency',max(20,ceil(numel(XTr)/256)), ...
  'ValidationPatience',5, ...
  'Plots','none','ExecutionEnvironment','auto','Verbose',false);

net = trainNetwork(XTr, YTrain, layers, opts);


        % (1) optional decimate to cut time in half/quarter
        decim = 4;                           % try 2 or 4
        XTrain_ds = cellfun(@(x) single(x(:,1:decim:end)), XTrain2,'uni',false);
        XVal_ds = cellfun(@(x) single(x(:,1:decim:end)),   XVal2,'uni',false);


        % fs = 200;  % recommended MEG downsample (340 Hz then decimate to ~250 Hz)
        % 
        % % 1) (optional) decimate raw cells BEFORE LV features (speeds up)
        % XTrain_ds = cellfun(@(x) single(x(:,1:2:end)), XTrain2, 'uni', false);
        % XVal_ds   = cellfun(@(x) single(x(:,1:2:end)), XVal2,   'uni', false);
        fs_ds = fs;  % set to your true post-decimation fs

        % 2) Build LV stacks [C×T×7]
        XTrainLV = cellfun(@(x) build_longview_features(x, fs_ds), XTrain_ds,'uni',false);
        XValLV   = cellfun(@(x) build_longview_features(x, fs_ds), XVal_ds,  'uni',false);

        % 3) Pack to fixed length L (short clip, e.g., 300600 ms of samples)
        L = round(prctile(cellfun(@(x) size(x,2), XTrainLV), 85));   % tighten for speed
        [trainImgs, trainLabs] = cell2imgs(XTrainLV, YTrain, L);     % returns [C×L×7×N]
        [valImgs,   valLabs]   = cell2imgs(XValLV,   YVal,   L);

        % 4) Keep class order + weights
        cats = categories(trainLabs);
        trainLabs = reordercats(trainLabs, cats);
        valLabs   = reordercats(valLabs,   cats);
        cnt = countcats(trainLabs); w = sum(cnt)./max(cnt,1); w = w/mean(w);
        clsLayer = classificationLayer('Name','cls','Classes',cats,'ClassWeights',w);


        C = size(trainImgs,1); L = size(trainImgs,2); D = size(trainImgs,3); % expect D=7
        lg = lv_cadenet_paper_layers(C, L, D, numel(cats), clsLayer);

        mb = 128;
        opts = trainingOptions('adam', ...
            'InitialLearnRate',3e-4, 'MaxEpochs',40, 'MiniBatchSize',mb, ...
            'Shuffle','every-epoch', ...
            'ValidationData',{valImgs, valLabs}, ...
            'ValidationFrequency', max(10, ceil(size(trainImgs,4)/mb)), ...
            'ValidationPatience', 8, ...
            'L2Regularization', 1e-4, ...      % AdamW-like weight decay proxy
    ...
    case 'data reduction'
        ...
        % fs = 500;
        % win = round(0.6*fs);                 % ~600 ms total window (tweak 400800 ms)
        % XTrain2c = cellfun(@(x) center_window(x, win), XTrain2, 'uni', false);
        % XVal2c   = cellfun(@(x) center_window(x, win), XVal2,   'uni', false);
        % 
        % [mdl, modelPCA, Ztr, Zva, cm, acc, bal] = train_pca_classifier( ...
        %     XTrain2c, YTrain, XVal2c, YVal, 'fs', fs, 'addBeta', true, 'var', 98);


        % TRAIN: compute scores
        [scores, ~] = compute_channel_scores(XTrain2, YTrain, 'fisher');  % or 'mi'/'rms'

        % Pick K channels (e.g., 128). If you have labels, pass them for mag/grad balance:
        % chLabels = refLbl;  % e.g., {'MEG0111','MEG0112','MEG0113',...}
        idxKeep = select_channels(scores, 128);                 % no labels
        % idxKeep = select_channels(scores, 128, chLabels);     % with labels

        % Prune train/val (MUST use the SAME idx for both!)
        XTrain_pr = prune_channels(XTrain2, idxKeep);
        XVal_pr   = prune_channels(XVal2,   idxKeep);

        % Now train your fast sequence model on the pruned data:
        decim  = 2;  % optional
        XTrain_fast = cellfun(@(x) single(x(:,1:decim:end)), XTrain_pr,'uni',false);
        XVal_fast   = cellfun(@(x) single(x(:,1:decim:end)), XVal_pr,  'uni',false);

        maxLen  = round(prctile(cellfun(@(x) size(x,2), XTrain_fast), 70));
        inputSz = size(XTrain_fast{1},1);                        % now = K (e.g., 128)
        cats = categories(YTrain);
        YTrain = reordercats(YTrain, cats); YVal = reordercats(YVal, cats);
        cnt = countcats(YTrain); w = sum(cnt)./max(cnt,1); w = w/mean(w);

        layers = [
            sequenceInputLayer(inputSz,'Normalization','none')
        ...
        opts = trainingOptions('sgdm','InitialLearnRate',5e-3,'Momentum',0.9, ...
            'MaxEpochs',150,'MiniBatchSize',256, ...
            'SequenceLength',maxLen,'SequencePaddingDirection','right', ...
            'ValidationData',{XVal_fast, YVal}, ...
            'ValidationFrequency',max(20,ceil(numel(XTrain_fast)/256)), ...
            'ValidationPatience',5,'Plots','none','ExecutionEnvironment','auto');
