...
%% ==================== COPY EVENT FILES TO REVIEWER FOLDERS ====================
% Folders observed in your tree:
%   /MEG_data/AHW_SpikeAnalysis/Reviewer/data/sss/<Reviewer>   (targets)
%   /MEG_data/AHW_SpikeAnalysis/Reviewer/Event_times            (event source)
%   /MEG_data/AHW_SpikeAnalysis/MEG_data/sss/Spike_times        (fallback)

rootDir         = '/MEG_data/AHW_SpikeAnalysis';
sheetDir        = fullfile(rootDir, 'Review_sheet');
destDataRoot    = fullfile(rootDir, 'Reviewer', 'data', 'sss');      % where reviewer folders live
eventDirs       = { fullfile(rootDir, 'Reviewer', 'Event_times'), ...
                    fullfile(rootDir, 'MEG_data', 'sss', 'Spike_times') };  % search order

useSymlinks       = false;   % false = real copies, true = ln -s
overwriteExisting = true;    % replace existing event files at destination
byRunSubfolder    = false;   % true -> .../<Reviewer>/<Subject>/RunXX/ ; false -> .../<Reviewer>/

% --- Load newest assignment CSV (Subject, RunNum, Physician1/2)
...
% Ensure we have RunNum (parse if needed)
if ~ismember('RunNum', T.Properties.VariableNames)
    if ismember('RunLabel', T.Properties.VariableNames)
        ...
        srcCol = []; if ismember('FileName',T.Properties.VariableNames), srcCol = T.FileName; end
        if isempty(srcCol) && ismember('FullPath', T.Properties.VariableNames), srcCol = string(T.FullPath); end
        assert(~isempty(srcCol), 'Need RunNum or parsable FileName/FullPath in assignment CSV.');
        parsed = regexp(srcCol, '_(?:run|Run)0?(\d+)_', 'tokens', 'once');
        T.RunNum = nan(height(T),1);
        for k=1:height(T), if ~isempty(parsed{k}), T.RunNum(k)=str2double(parsed{k}{1}); end, end
...
% Unique jobs so we don’t duplicate work
[~, uix] = unique(Rows(:, {'Reviewer','Subject','RunNum'}), 'rows', 'stable');
Jobs = Rows(uix, :);

% Ensure reviewer folders exist
revs = unique(Jobs.Reviewer);
for r = 1:numel(revs)
    rdir = fullfile(destDataRoot, revs(r));
    if ~exist(rdir, 'dir'), mkdir(rdir); end
end

% --- Copy/link matching event files
nOK=0; nMiss=0; nFail=0;
logRows = strings(0,6);  % Reviewer,Subject,Run,Source,Destination,Action
for k = 1:height(Jobs)
    ...
    dstBase = fullfile(destDataRoot, rev);
    if byRunSubfolder
        dstBase = fullfile(dstBase, subj, sprintf('Run%02d', runN));
    end
    if ~exist(dstBase, 'dir'), mkdir(dstBase); end

    % Find matching event files in sources (first hit wins)
    files = []; srcBase = "";
    for s = 1:numel(eventDirs)
        L = local_find_event_files(eventDirs{s}, subj, runN);
        if ~isempty(L)
            files = L; srcBase = string(eventDirs{s});
            break;
        end
    end

    if isempty(files)
        warning('EVENT MISSING: %s Run%d', subj, runN);
        nMiss = nMiss + 1;
        logRows(end+1,:) = [rev, subj, string(runN), "", "", "MISSING"]; %#ok<SAGROW>
        continue;
    end

    % Place all matches (usually one per run)
    for f = 1:numel(files)
        src = fullfile(char(srcBase), files(f).name);
        dst = fullfile(char(dstBase), files(f).name);

        if useSymlinks
            if overwriteExisting && exist(dst,'file'), delete(dst); end
            [status,msg] = system(sprintf('ln -s "%s" "%s"', src, dst));
            if status ~= 0
                warning('Symlink failed (%s). Falling back to copy.', strtrim(msg));
                [ok,msg2] = copyfile(src, dst, 'f');
                if ok, nOK=nOK+1; action="COPY(FB)"; else, nFail=nFail+1; action="FAIL"; end
            else
                nOK=nOK+1; action="LINK";
            end
        else
            if ~overwriteExisting && exist(dst,'file')
                action = "SKIP";
            else
                if overwriteExisting
                    [ok,msg2] = copyfile(src, dst, 'f');
                else
                    [ok,msg2] = copyfile(src, dst);
                end
                if ok, nOK=nOK+1; action="COPY";
                else, nFail=nFail+1; warning('Copy failed: %s -> %s (%s)', src,dst,msg2); action="FAIL";
                end
            end
        end
...
% --- Summary + log
fprintf('Event files: placed %d, missing %d subject/run, %d failed.\n', nOK, nMiss, nFail);
logT = array2table(logRows, 'VariableNames', {'Reviewer','Subject','Run','Source','Destination','Action'});
writetable(logT, fullfile(sheetDir, 'copy_events_log.csv'));

%% ==================== helpers ====================
function files = local_find_event_files(eventDir, subject, runNum)
% Return dir() entries for event files matching Subject + Run.
% Handles "Run6" / "Run06" and "run" vs "Run".
    subjEsc = regexptranslate('escape', char(subject));
    rx = sprintf('^%s_(?:run|Run)0?%d_.*_spk_DS\\.eve$', subjEsc, runNum);
    L  = dir(fullfile(eventDir, sprintf('%s_*', char(subject))));
    if isempty(L), L = dir(fullfile(eventDir, '*.eve')); end % fallback scan (slower, but robust)
    names = {L.name};
    keep  = ~cellfun('isempty', regexpi(names, rx, 'once'));
    files = L(keep);
    % Dedup just in case
    if ~isempty(files)
        [~,ia] = unique({files.name}, 'stable'); files = files(ia);
    end
end
