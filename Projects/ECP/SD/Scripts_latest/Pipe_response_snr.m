...
%% Create a folder
folderPath = fullfile(save_dir,'subj');
...
if LI_method == 1
    ...
    for subj = 1:size(pow_hc.left, 2)
        ...
        cfg = []; cfg.outdir = folderPath; filename = ['Power_hc_Subj_' num2str(subj)]; cfg.filename = filename; cfg.type = 'fig'; do_export_fig(cfg);
        pause (1)
        close all
    end
    
    % Loop through each subject instead of calculating mean
    for subj = 1:size(pow_pt.left, 2)
        cfg_pow = [];
        cfg_pow.labels = net_sel_mutiple_label(network_sel);
        cfg_pow.colors = colr;
        cfg_pow.wi = wi;
        ...
        % Export figs for each subject
        cfg = []; cfg.outdir = folderPath; filename = ['Power_pt_Subj_' num2str(subj)]; cfg.filename = filename; cfg.type = 'fig'; do_export_fig(cfg);
        pause (1)
        close all
    end
end
