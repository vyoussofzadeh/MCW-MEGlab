...
%% Lat analysis (whole-brain; all times)
cfg = [];
...
cfg.lat_index = [idx_L, idx_R];
cfg.fplot = 0;
[LI_anim_hc,~] = do_lat_analysis(cfg);
cfg.sinput = sFiles_anim_pt;
[LI_anim_pt,~] = do_lat_analysis(cfg);
...
% set(gca,'Xtick', 1:length(wi),'XtickLabel',linspace(wi(1),wi(end),length(wi)));
set(gca,'FontSize',8,'XTickLabelRotation',90);
set(gcf, 'Position', [1000   400   1500   500]);
title(['Anim, LI, HC vs PT']), set(gca,'color','none');
legend({'HC=27','PT=72'})
xlabel('Time interval (sec)')
ylabel('LI values')
% subplot 212
% plot(LI_anim_hc - LI_anim_pt)

%% Symb (PT vs. HC)
% clc, close all
cfg = [];
cfg.thre = 0;
cfg.sinput = sFiles_symb_hc;
cfg.BS_data_dir = BS_data_dir;
cfg.atlas = atlas;
cfg.wi = wi;
cfg.lat_index = [idx_L, idx_R];
cfg.fplot = 0;
[LI_symb_hc,~] = do_lat_analysis(cfg);
cfg.sinput = sFiles_symb_pt;
[LI_symb_patn,~] = do_lat_analysis(cfg);

figure,plot(LI_symb_hc)
hold on,
plot(LI_symb_patn,'r')
val = round(mean(wi(:,1),2),2);
set(gca,'Xtick', 1:2:length(wi),'XtickLabel',val(1:2:end));
set(gca,'FontSize',8,'XTickLabelRotation',90);
set(gcf, 'Position', [1000   400   1500   500]);
title('Symb, LI,HC vs PT'), set(gca,'color','none');
legend({'HC=27','PT=72'})
xlabel('Time interval (sec)')
ylabel('LI values')
...
%% Plot source maps
ssinput = load(fullfile(BS_data_dir,sFiles_anim_hc));
...
%% LI Subjects (whole-brain, all times)
close all
cfg = [];
cfg.sinput = sFiles_anim_hc;
cfg.BS_data_dir = BS_data_dir;
...
ft_progress('init', 'text',     'please wait ...');
clear m_LI_sub m_LI_sub_max
for i=1:length(sFiles_in)
    ft_progress(i/length(sFiles_in), 'Processing subjects %d from %d', i, length(sFiles_in));
    pause(0.1);
    cfg.sinput = sFiles_in{i};
    cfg.wi = wi_sub_max(i,:);
    [LI,~, ~] = do_lat_analysis(cfg);
    LI_sub{i} = LI;
    m_LI_sub_max(i) = mean(LI);
end
ft_progress('close')
...
cfg = []; 
cfg.sub_sel = sub_sel; 
cfg.d_in = m_LI_sub_max;
cfg.tit = ['LIs (max LI): ', taskcon{J}, '-', subcon];
do_barplot_LI(cfg)

%%
tmp = load(fullfile(BS_data_dir,sinput));
...
cfg.title = ['']; cfg.alpha = 1; cfg.coor = [];
cfg.surf = src;
cfg.d_in = mean(tmp.ImageGridAmp,2);
do_surfplot(cfg);

%% Inspecting source maps (in time)
Run_sourcemap_time % sources over time - all
Run_sourcemap_time_window % over time intervals

toi_pst = LI_max; % Sel time intervals
Run_sourcemap_time_sel_window
...
cfg.net_sel = net_sel; 
[opt_idx23_L,opt_idx23_R] = do_network_indecies(cfg);
...
cfg.thre = 0; 
cfg.wi = wi;
cfg.lat_index = [opt_idx23_L; opt_idx23_R]'; 
cfg.fplot = 1;
cfg.tit = net_tag;
...
%% Subject-level LI
net_sel_mutiple_label = {'frontal', 'temporal', 'parietal', 'front-temporal', 'all'};
