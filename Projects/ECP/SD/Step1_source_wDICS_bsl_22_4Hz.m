    ...
    for ii = 1:length(subj)
        ...
        if ~(contains(subj{ii},no_anat) || contains(subj{ii},incomplete_data))
            ...
            for jj=1:length(dd)
                tmp = load(dd(jj).name);
                disp(tmp.Comment);
                if contains(tmp.Comment,['22Hz_wdics_', atag{kkk}])
                    sel = [sel,jj];
                end
            end
            ...
            if length(sel)==2
                ...
                if ~isempty(dd1)
                    for kk=1:length(dd1)
                        ...
                        disp(tmp.Comment);
                        if contains(tmp.Comment,[subj{ii}, '_22Hz_wdics_', atag{kkk}])
                            runok = 0; break,
                        else
                            runok = 1;
                        end
...
%                     pause
                    bst_process('CallProcess', 'process_average', sFiles1, [], ...
                        ...
                        'weighted',        0, ...
                        'Comment', [subj{ii}, '_22Hz_wdics_', atag{kkk}], ...
                        'scalenormalized', 0);
                else
...
for kkk = 1:length(atag)
    ...
    for ii=1:length(dd3)
        ...
        if contains(comm_data{ii}, ['22Hz_wdics_', atag{kkk}])
            sel = [sel,ii];
...
%             pause
    ...
    dd = rdir(fullfile (BS_data_dir,'/Group*/wDICS_baseline_22_4/results*.mat'));
    ...
    for ii=1:length(dd)
        ...
        if contains(comm_data{ii}, ['22Hz_wdics_', atag{kkk}])
            ...
            dconn{kk} = tkz{2};
            sFiles_name_completed{kk} = [subj_comp{kk},'_',dconn{kk}, '_', tkz{3}(1)];
            kk=1+kk;
        end
    end
    ...
    idx = find(contains(sFiles_name,'22Hz_wdics')==1);
