...
for i = 1:length(uniqueROIs)
    ...
    newRow = {bestMethod, roi, bestCorrelation, bestConcordance, bestTimeInterval, bestDiscordSubs, bestDiscordSubs_megli, bestDiscordSubs_optmegli, bestDiscordSubs_fmrili, bestDiscordSubs_rSNR_L, bestDiscordSubs_rSNR_R};
    bestResultsTable_fixed = [bestResultsTable_fixed; newRow];
end

% Set column names for the best results table
bestResultsTable_fixed.Properties.VariableNames = {'Best_LI_Method', 'ROI', 'Best_Correlation', 'Best_Concordance', 'Best_Time_Interval', 'Best_Discord_Subs', 'MEG_LI', 'optMEG_LI', 'fMRI_LI', 'rSNR_L', 'rSNR_R'};
...
writetable(bestResultsTable_save, fullfile(save_dir,'Best_LI_Methods_Summary_fixed.csv'));


% Save best results table as text file
fid = fopen(fullfile(save_dir,'Best_LI_Methods_Summary_fixed.txt'), 'wt');
