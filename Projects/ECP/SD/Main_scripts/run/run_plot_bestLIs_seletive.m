...
for i = 3:3%length(bestResultsTable.ROI)
    ...
    plot_max_rSNR(rsnr_max, discordantSubs)
    if plot_option == 1
        %     sgtitle(sprintf('Max rSNR Discordant ROI: %s, Method: %s', roi, method));
        cfg = []; cfg.outdir = save_dir; filename = sprintf('Selective Max rSNR Discordant ROI: %s, Method: %s', roi, method); cfg.filename = filename; cfg.type = 'svg'; do_export_fig(cfg); close all, combined_path = fullfile(save_dir,[cfg.filename, '.svg']); web(combined_path, '-new');
    end
    ...
    if plot_option == 1
        cfg = []; cfg.outdir = save_dir; filename = sprintf('Selective ReactionTime Discordant ROI: %s, Method: %s', roi, method); cfg.filename = filename; cfg.type = 'svg'; do_export_fig(cfg); close all, combined_path = fullfile(save_dir,[cfg.filename, '.svg']); web(combined_path, '-new');
    end
    ...
    if plot_option == 1
        cfg = []; cfg.outdir = save_dir; filename = sprintf('Selective Task performace ROI2: %s, Method: %s', roi, method); cfg.filename = filename; cfg.type = 'svg'; do_export_fig(cfg); close all, combined_path = fullfile(save_dir,[cfg.filename, '.svg']); web(combined_path, '-new');
    end
    
    plotCorrResponseAccReactionTime(accuracyResults_updt, T_patn_MEGfMRI)
    if plot_option == 1
        cfg = []; cfg.outdir = save_dir; filename = sprintf('Selective Corr_ReactionTvsAcc_: %s, Method: %s', roi, method); cfg.filename = filename; cfg.type = 'svg'; do_export_fig(cfg); close all, combined_path = fullfile(save_dir,[cfg.filename, '.svg']); web(combined_path, '-new');
    end
end
cd(save_dir)
