function Data_hcp_atlas = ecpfunc_hcp_atlas4(cfg_main)
...
idx_sel_L = strcmp(rois(all_idx_L), 'L_46_ROI L') | strcmp(rois(all_idx_L), 'L_9-46d_ROI L');
idx_sel_R = strcmp(rois(all_idx_L), 'R_46_ROI R') | strcmp(rois(all_idx_L), 'R_9-46d_ROI R');
...
for i=1:length(ATG_labels)
    ATG_L_label = [ATG_L_label; rois(contains(rois, ATG_labels{i}))];
    ATG_R_label = [ATG_R_label; rois(contains(rois, strrep(ATG_labels{i}, 'L_', 'R_')))];
end
...
for i=1:length(PSTG_labels)
    PSTG_L_label = [PSTG_L_label; rois(contains(rois, PSTG_labels{i}))];
    PSTG_R_label = [PSTG_R_label; rois(contains(rois, strrep(PSTG_labels{i}, 'L_', 'R_')))];
end
...
%% Lateral rois
LI_glasser_lateral_rois = load(BS_atlas);
