...
%%
savedir = '/data/MEG/Vahab/Github/MCW_MEGlab/MCW_MEGlab_git/Projects/ECP/SD/data_info';

cd(savedir)
if exist(fullfile(savedir,'comments.mat'),'file') == 2
    load(fullfile(savedir,'comments.mat')),
else
    Comment = []; k=1; kk=1;
    need_correction = [];
    ...
    save(fullfile(savedir,'comments.mat'),'Comment'),
end
...
%%
cd(savedir)
if exist(fullfile(savedir,'subjects.mat'),'file') == 2
    load(fullfile(savedir,'subjects.mat')),
else
    ...
    save(fullfile(savedir,'subjects.mat'),'subjs_2','subjs_3'),
end
...
%% choose atlas
cd('//data/MEG/Research/ECP/Story_Math/BS_source')
clear mdk_roi parcel_all

disp('1: Desikan-Killiany_68')
disp('2: Mindboggle_62');
disp('3: Harvard Oxford_94');
disp('4: Destrieux_148');
atlas_sel = input('Select data condition:');
switch atlas_sel
    case 1
        atlas = load('scout_Desikan-Killiany_68.mat'); atag = 'DK';
    case 2
        atlas = load('scout_Mindboggle_62.mat'); atag = 'MB';
    case 3
        %         atlas = load('scout_HO_update_94.mat');
        atlas = load('/data/MEG/Vahab/Github/MCW_MEGlab/FT/Atlas/HO/scout_HO_update_94.mat');
        run('/data/MEG/Vahab/Github/MCW_MEGlab/FT/Atlas/HO/HO_fulllabels.m')
        atag = 'HO';
        %         HA_roi2
        disp(roi_ha2)
    case 4
        atlas = load('scout_Destrieux_148.mat'); atag = 'DSX';
end
...
wi = [];
w1 = 0; l = 0.05; ov = 0.01; j=1; %ov = l.*0.3
while w1+l < 2
    wi(j,:) = [w1, w1+l]; j=j+1; w1 = w1 + ov;
end
length(wi)

sFiles_anim_hc = 'Group_analysis/@intra/results_average_221221_1326.mat';
sFiles_anim_pt = 'Group_analysis/@intra/results_average_221221_1336.mat';

sFiles_symb_hc = 'Group_analysis/@intra/results_average_221226_1141.mat';
sFiles_symb_pt = 'Group_analysis/@intra/results_average_221222_1406.mat';

sFiles_anim_hc_sFiles_anim_pt = 'Group_analysis/@intra/results_221226_1142.mat';

input = sFiles_anim_hc;
input = sFiles_anim_pt;

input = sFiles_symb_hc;
input = sFiles_symb_pt;

% input = sFiles_anim_hc_sFiles_anim_pt;
...
for j=1:length(wi)
    ...
    [roiid, idx, roi_val] = do_barplot_ecp(parcelval,roi_ha2', 0.55, 2);
...
% grid on, 
% grid minor
set(gca,'Xtick', 1:length(wi),'XtickLabel',mean(wi(:,1),2));
...
for i=1:length(sFiles_in)
    disp([num2str(i), ':', num2str(length(sFiles_in))])
    tmp = load(sFiles_in{i});
    sub{i} = tmp.HeadModelFile(1:6);
    LI = [];
    for j=1:length(wi)
        timind1 = nearest(tmp.Time, wi(j,1));
        timind2 = nearest(tmp.Time, wi(j,2));
