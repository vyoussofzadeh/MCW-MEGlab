...
temp_indir = '/data/MEG/Research/ECP/Semantic_Decision/temp_MEGnet';
%- Output dir
...
if exist(['datalog_ic_',tag,'.mat'], 'file') == 2
    ...
    clear datafolder datafile subj_all sub
    datafile_fif = [];
    dirs = {indir, temp_indir}; % Combine both directories in a cell array
%         dirs = {temp_indir}; % Combine both directories in a cell array
    for k = 1:length(dirs)
        d = rdir([dirs{k}, '/**/ICA/*', tag, '*/ec*_ica_clean.fif']);
        for i = 1:length(d)
            [pathstr, name] = fileparts(d(i).name);
            datafolder{i} = pathstr;
            datafile{i} = d(i).name;
    ...
    datafile_fif = unique(datafile_fif'); % Remove duplicates and transpose
    save(['datalog_ic_',tag,'.mat'],'datafile_fif','subj_all', 'sub')
end
disp(datafile_fif)
