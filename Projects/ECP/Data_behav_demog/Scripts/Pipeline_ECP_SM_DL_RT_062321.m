...
for dd = 1:size(datafile2,1)
    ...
    %%
    str_run = trlInfoColDescr1(idx_str,3); str_run_uq = unique(str_run);
    for iq = 1:length(str_run_uq)
        idx = find(str_run == str_run_uq(iq));
        idx_str_run_uq(iq) = idx(1);
    end
    
    math_run = trlInfoColDescr1(idx_math,3);math_run_uq = unique(math_run);
    for iq = 1:length(math_run_uq)
        idx = find(math_run == math_run_uq(iq));
        idx_math_run_uq(iq) = idx(1);
    end
...
%         disp(['Accuracy of math was: %', num2str(accuracy_math)]);
        ...
        accuracy_str  = length(find(trlInfoColDescr1(idx_str_run_uq,18)==1))/length(idx_str_run_uq)*100;
        accuracy_math = length(find(trlInfoColDescr1(idx_math_run_uq,18)==1))/length(idx_math_run_uq)*100;
        
        %         test = trlInfoColDescr1(idx_math,18);
        %         newData=test(~isnan(test));
        %         accuracy_math = length(find(newData==1))/length(newData)*100;        
