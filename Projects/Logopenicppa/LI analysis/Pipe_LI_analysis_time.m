...
cd(BS_data_dir)
dd = rdir(fullfile(BS_data_dir,'/data/Group_analysis/smoothed/results*.mat'));
dd1 = rdir(fullfile(BS_data_dir,'/data/Group_analysis/smoothed/results*_mean.mat'));
dd2 = rdir(fullfile(BS_data_dir,'/data/Group_analysis/smoothed/results*average*.mat'));

list1 = {dd.name}; list2 = {dd1.name}; list3 = {dd2.name};

dd4 = setdiff(list1, list2); dd5 = setdiff(dd4, list3);

for jj=1:length(dd5), disp([num2str(jj),':',dd5{jj}]); end

sFiles_name = []; for jj=1:length(dd5), sFiles_name{jj} = fullfile(dd5{jj}); end
...
disp('----')
disp(data_info)
save(fullfile(data_info_dir,'comments_subject_new.mat'),'data_info'),
...
for i=1:length(data_info_new)
    ...
    cfg_in.svdir = '/data/MEG/Research/logopenicppa/results/CRM/LI';
