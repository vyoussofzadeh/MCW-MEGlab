...
%     conn_method = 'amplcorr';
    ...
    savefile_seed_conn = fullfile(subdir,['semanticnet_conn_',subj,'_run_', run, '_', conn_method, '.mat']);
%     num2str(foi(1)), ...
%         '-', num2str(foi(2)), 'Hz.mat']); 
    if exist(mripfile,'file')== 2 %% && exist(savefile_seed_conn,'file') ~=2
        ...
        cfg.roiidx = roiidx;
        net_conn_seed = do_conn_semantic(cfg, cln_data);
