...
while flag.analysis == 1
    ...
    print(gcf, '-dpdf',['./results/source_all','_', date,'.pdf']);
    ...
    if pka==1
        ...
        for j = 1:npeaks
            ...
            print(gcf, '-dpdf', ['./results/source_all_p', num2str(j),'_', date,'.pdf']);
            %         Run_pdf_save
        end
    end
    ...
    if ~isempty(toccur)
        ...
        for i=1:length(keep_matrix)
            ...
            cfg.continuous = 'yes';
            ft_databrowser(cfg, data_spk);
            print(gcf, '-dpdf', ['./results/sensor_t', num2str(i),'_', date,'.pdf']);
        end
        ...
        if ETS_ask ==1
            ...
            for j=1:length(un_time_occur_keep)
                ...
                print(gcf, '-dpdf', ['./results/smap_',tsel{j}(3:end),'_', date,'.pdf']);
                ...
                print(gcf, '-dpdf', ['./results/timeseries_',tsel{j}(3:end),'_', date,'.pdf']);
                ...
                print(gcf, '-dpdf', ['./results/topo_',tsel{j}(3:end),'_', date,'.pdf']);
            end
        end
        ...
        if savingevent_ask == 1
            source_img_dir = ['./results/*',date,'.pdf'];
            %     dest_pdf_dir = ['subject_pdfs/' [subj,'_', run, '_',date,'.pdf']];
            
            if exist(['./results/results_', date,'.pdf'],'file') == 2
                delete(['./results/results_', date,'.pdf'])
            end
            ...
            append_pdfs(['./results/results_', date,'.pdf'], tmp{:})
            for i=1:length(tmp)
                delete(tmp{i})
            end
        end
        ...
        if savingevent_ask ==1
            ...
            tevent = table(event);
            tevent(1,1) = 'test';
            tevent.Properties.VariableNames{'Var1'} = 'Name';
            ...
            savefile_readme  = ['Readme_', subj,'_run', num2str(run), '_date', date,'.txt'];
            ...
            dlmwrite(savefile_readme, data_sel,'delimiter','');
