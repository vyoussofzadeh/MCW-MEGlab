...
sProcess.Comment     = 'FieldTrip: ft_sourceanalysis MNE ICA v052821';
...
for iChanFile = 1:1%length(AllChannelFiles)
    ...
    for iInput = 1:1%length(iChanInputs)
        ...
        %%
        toi = [fcln_data.time{1}(1),fcln_data.time{1}(end)];
        warning(['Maximum trial length:[', num2str(toi(1)), ',', num2str(toi(2)),']']);
        disp('OK to proceed with full-length data: 1, No, another time interval: 2:');
        ask_time = input(':');
        if ask_time == 2
            disp('Enter time interval in sec, eg, [0.7,1.2];');
            toi = input(':');
            fcln_data = vy_epoch(fcln_data, toi);
        end
        ...
        for i=1:length(nIC)
            ...
            ResultsMat.Comment       = ['ft_sourceanalysis:' Method, '_IC', num2str(nIC(i)), '_', num2str(foi(1)),'_',num2str(foi(2)),'_Hz_', num2str(toi(1)),'_',num2str(toi(2)),'_sec_', datestr(now, 'dd/mm/yy-HH:MM')];
...
cfg = [];
cfg.toilim = toi;
ep_data = ft_redefinetrial(cfg, r_data);
