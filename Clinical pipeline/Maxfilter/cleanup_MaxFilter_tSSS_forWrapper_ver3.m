function cleanup_MaxFilter_tSSS_forWrapper_ver3(subjIDpath, outDirpath)
...
for j = 1:numel(files)
    ...
    hasCHPI = ~isempty(regexpi(rawFile, 'c[\s_-]*hpi'));
    ...
    %             if hasCHPI
    %                 fprintf('Detected cHPI in filename -> running tSSS + movecomp\n');
    %                 runMaxFilterTSSS_movecomp(subjIDpath, rawFile, FN.tsssFile, FN.dir, baseRun);
    %             else
    %                 fprintf('No cHPI detected in filename -> running standard tSSS\n');
    ...
    if isEmpty
        ...
        if exist(FN.tsssFile, 'file') ~= 2
            if hasCHPI
                fprintf('Detected cHPI in filename -> trying tSSS + movecomp\n');
                try
                    runMaxFilterTSSS_movecomp(subjIDpath, rawFile, FN.tsssFile, FN.dir, baseRun);
                catch ME
                    msg = ME.message;
                    
                    if ~isempty(regexpi(msg, 'continuous HPI was not active')) || ...
                            ~isempty(regexpi(msg, 'signal 11')) || ...
                            ~isempty(regexpi(msg, 'core dump')) || ...
                            ~isempty(regexpi(msg, 'output file.*was not written'))
                        
                        fprintf('Movecomp failed for this run -> falling back to standard tSSS\n');
                        runMaxFilterTSSS(subjIDpath, rawFile, FN.tsssFile);
                    else
                        rethrow(ME);
                    end
...
function runMaxFilterTSSS_movecomp(inDir, rawFileName, outFileFull, runDir, baseRun)
inputFile = fullfile(inDir, rawFileName);
hpFile    = fullfile(runDir, [baseRun '_headpos.txt']);

fprintf('Running MaxFilter tSSS + movecomp:\n  %s\n  -> %s\n', inputFile, outFileFull);
fprintf('Head position file:\n  %s\n', hpFile);

cmd = sprintf(['/neuro/bin/util/maxfilter -f %s -o %s ' ...
    '-ctc /neuro/databases/ctc/ct_sparse.fif ' ...
    '-cal /neuro/databases/sss/sss_cal.dat ' ...
    '-autobad off -st 10 -corr 0.9 ' ...
    '-movecomp inter -hp %s -hpisubt amp -force'], ...
    inputFile, outFileFull, hpFile);

[status, cmdout] = system(cmd);

if status ~= 0
    error('MaxFilter tSSS + movecomp failed:\n%s', cmdout);
end

if exist(hpFile, 'file') ~= 2
    warning('Movecomp finished but head position file was not created: %s', hpFile);
end
end
